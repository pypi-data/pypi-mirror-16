...
class FMQuery(object):
	...
	def run_script(self, name, param=None):
		"""
			Run the named FM script to be run after the query
			
			If given, the `param` arg should be a single string.
		"""
		self._params['-script'] = name
		if param is not None:
			self._params['-script.param'] = param
		return self
		
	def run_script_prefind(self, name, param=None):
		"""
			Run the named FM script to be run before a find
			
			If given, the `param` arg should be a single string.
		"""
		self._params['-script.prefind'] = name
		if param is not None:
			self._params['-script.prefind.param'] = param
		return self
		
	def run_script_presort(self, name, param=None):
		"""
			Run the named FM script to be run before sorting results
			
			If given, the `param` arg should be a single string.
		"""
		self._params['-script.presort'] = name
		if param is not None:
			self._params['-script.presort.param'] = param
		return self
