...
class FMLayout(object):
	...
	def get_write_query(self, recid=None):
		"""
			Provides an FMEditQuery on this layout.
			
			Will return an "edit" query if the recid is given,
			otherwise a "new" one.
		"""
		return query.FMEditQuery(recid=recid, server=self._server, db=self.db, layout=self.id)
