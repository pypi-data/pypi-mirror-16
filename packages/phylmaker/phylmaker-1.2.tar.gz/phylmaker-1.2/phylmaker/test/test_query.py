...
class QueryScriptTests(base.BaseTestCase):
	"""
		Test support for triggering scripts with queries
	"""
	def setUp(self, **kwargs):
		super(QueryScriptTests, self).setUp(**kwargs)
		self.query = phylmaker.query.FMFindQuery(db='foo', layout='bar')
		
	
	def test_script(self):
		# Test that a script can be specified
		self.query.run_script('frob')
		
		params = self.query.to_dict()
		assert params['-script'] == 'frob'
		
	def test_param(self):
		# Test that a parameter can be passed to a script
		self.query.run_script('frob', 'something')
		
		params = self.query.to_dict()
		assert params['-script'] == 'frob'
		assert params['-script.param'] == 'something'
		
	def test_param_named(self):
		# Test the script parameter keyword argument names
		self.query.run_script(name='frob', param='something')
		
		params = self.query.to_dict()
		assert params['-script'] == 'frob'
		assert params['-script.param'] == 'something'
		
	def test_prefind(self):
		# Test that a pre-find script can be specified
		self.query.run_script_prefind('frob')
		
		params = self.query.to_dict()
		assert params['-script.prefind'] == 'frob'
		
	def test_prefind_param(self):
		# Test that a pre-find script can be given a parameter
		self.query.run_script_prefind(name='frob', param='summert')
		
		params = self.query.to_dict()
		assert params['-script.prefind'] == 'frob'
		assert params['-script.prefind.param'] == 'summert'
		
	def test_presort(self):
		# Test that a pre-sort script can be specified
		self.query.run_script_presort('frob')
		
		params = self.query.to_dict()
		assert params['-script.presort'] == 'frob'
		
	def test_presort_param(self):
		# Test that a pre-sort script can be given a parameter
		self.query.run_script_presort(name='frob', param='summert')
		
		params = self.query.to_dict()
		assert params['-script.presort'] == 'frob'
		assert params['-script.presort.param'] == 'summert'
		
	def test_new_script(self):
		# Test that a new operation can be given a script
		query = phylmaker.query.FMEditQuery(recid=None, db='foo', layout='bar')
		query.run_script('baz', 'fez')
		
		params = query.to_dict()
		assert params['-script'] == 'baz'
		assert params['-script.param'] == 'fez'
		
	def test_edit_script(self):
		# Test that an edit operation can be given a script
		query = phylmaker.query.FMEditQuery(recid=123, db='foo', layout='bar')
		query.run_script('baz', 'fez')
	...
	def test_delete_script(self):
		# Test that a delete operation can be given a script
		self.query.delete(123)
		self.query.run_script('baz', 'fez')
