...
def connect_to_region(region, **connect_args):
    try:
        conn = ECSConnection(conn.acs_access_key=connect_args.get('acs_access_key'), conn.acs_secret_access_key=connect_args.get('acs_secret_access_key'), conn.region=region, conn.security_token=connect_args.get('security_token', None))
