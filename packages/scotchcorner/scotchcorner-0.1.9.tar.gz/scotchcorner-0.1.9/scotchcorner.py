...
class scotchcorner:
    ...
    def _format_axes(self):
        ...
        for i, ax in enumerate(self.histhori):
            nbins = max([len(ax.get_yticklabels()), 5]) # make sure there are at least 4 tick marks (after removal of one)
            ax.xaxis.set_major_locator(LinearLocator(numticks=nbins))
        ...
        for i, ax in enumerate(self.histvert):
            ...
            nbins = max([len(ax.get_yticklabels()), 5]) # make sure there are at least 4 tick marks (after removal of one)
            ax.yaxis.set_major_locator(LinearLocator(numticks=nbins))
