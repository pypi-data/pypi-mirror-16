...
Copyright 2003-2006 Peter Erwin
...
#   PROXY NOTE (may be out-of-date or incorrect):
#   To use this on a network connected to the Web via proxy,
# you need the environment variable HTTP_PROXY defined properly.
# If it's not already defined, you can do it as follows (substitute your
# proxy URL as appropriate):
#   unix-[t]csh-> setenv HTTP_PROXY=http://proxy.ll.iac.es:3128
# OR:
#   unix-bash-> http_proxy="HTTP_PROXY=http://proxy.ll.iac.es:3128"
#   unix-bash-> export http_proxy
# It appears this is *already* defined for IAC accounts, so we don't have to
# worry about this -- hurray!
#
#
#
...
# However, the UKIRT, CFHT, and AAT archives fail to catch this, and produce lines
# like this:
...
#     [] SMOKA -- Subaru plus Japanese 1 and 1.9m telescopes
...
# we are a Netscape 4.5-class browser, rather than its default description
...
def SearchOneArchive( archive, targetName, debugState=0, saveSuccesses=0 ):
	...
	try:
		...
		if ( nDataFound > 0 ):
			...
			if ( saveSuccesses ):
				prefix = string.replace(targetName, ' ', '_')
...
def DoSearch( targetName, archiveList, debugSetting = 0, doThreading = 1 ):
...
#	print "            --esohst : use ESO's HST archive in Europe [default]"
#	print "            --stsci : use STScI HST archive"
...
def main(argv):
	# Set DEBUG_DEFAULT = 1 to enable saving of *all* web pages returned:
	DEBUG_DEFAULT = 0
	multipleSearches = 0
	whichHSTarchive = kHST_ESO
# 	doAAT = 1
# 	doSDSS = 1
	doGetCoords = 1   # do we need to do a Simbad coordinate lookup?
	                  # (default = yes because by default we search SDSS archive)
	doThreading = 1   # default is to use threads
	archive_module_list = None   # None (default) = load the default set of archives


	# Set up target, etc, by parsing command-line:
	doCoordSearch = 0     # are we searching by (user-supplied) coordinates?
	coordsList = NO_COORDS
	targetName = NO_NAME
	noTargetName = 1
	noBoxSize = 1

	usageString =  "telarchive [options] \"target_name\" [box_size_in_arcmin]"
	parser = optparse.OptionParser(usage=usageString, version="%prog " + kVersionNumber)

	parser.add_option("-d", "--debug", type="int", dest="debugState", default=DEBUG_DEFAULT)
	parser.add_option("-c", "--coords", type="string", dest="coords", default=NO_COORDS)
	parser.add_option("--archives", action="store_true", dest="printArchiveList", default=False)
	parser.add_option("--nothreads", action="store_false", dest="useThreads", default=True)
	parser.add_option("--threads", action="store_true", dest="useThreads")
	parser.add_option("--noaat", action="store_false", dest="useAAT", default=True)
	parser.add_option("--nosdss", action="store_false", dest="useSDSS", default=True)
	parser.add_option("--usearchive", type="string", dest="requestedArchive", default="all")
	parser.add_option("--targetfile", type="string", dest="targeFile", default="none")
	parser.add_option("--coordsfile", type="string", dest="coordsFile", default="none")
	
	try:
		(options, positional_arguments) = parser.parse_args(argv)
	except:
		print "Something wrong with the input arguments..."
		PrintHelp()
		sys.exit(2)
	
	
	# missing: how to handle -h, --help
	# missing: how to handle -v, --version
	# missing: "--targetfile" handling
	# missing: "--coordsfile" handling

	if options.printArchiveList:
		PrintArchiveList()
		sys.exit()
	doThreading = options.useThreads
	if (doThreading == 1) and (threadingPresent == 0):
		doThreading = 0
		print "Multithreading not available..."		
	if options.requestedArchive != "all":
		archive_module_list = [ moduel_list.shorthand_dict[options.requestedArchive] ]
	if options.useAAT is False:
		print "Ignoring Anglo-Australian Telescope..."
	if options.useSDSS is False:
		print "Ignoring Sloan Digital Sky Survey..."
	if options.coords != NO_COORDS:
		try:
			doCoordSearch = 1
			doGetCoords = 0
...
# 	shortOptionString = "hvc:d"
# 	longOptionList = ["help", "version", "debug", "coords=", "archives", "nothreads", 
# 						"threads", "noaat", "noAAT", 
# 						"nosdss", "noSDSS", "usearchive=", "targetfile=", "coordsfile="]
# 	try:
# 		(options, arguments) = getopt.getopt(argv[1:], shortOptionString, longOptionList)
...
# 			sys.exit()
# 		elif opt == "--targetfile":
# 			targetListFile = optarg
# 			if (not os.path.exists(targetListFile)):
...
# 			targetListFile = optarg
# 			if (not os.path.exists(targetListFile)):
# 				print "\n   Can't find file \"%s\" containing coordinate list\n" % targetListFile
# 				sys.exit(1)
# 			multipleSearches = 1
# 			doCoordSearch = 1
# 			doGetCoords = 0
# 		elif opt in ["-d", "--debug"]:
# 			DEBUG = 1
# 			print "Debugging on (all returned HTML will be saved to files)..."
# 		elif opt == "--nothreads":
...
# 			if threadingPresent:
# 				doThreading = 1
# 				print "Multithreading active..."
# 			else:
# 				doThreading = 0
# 				print "Multithreading not available..."
# 		elif opt in ["--noaat", "--noAAT"]:
# 			print "Ignoring Anglo-Australian Telescope..."
# 			doAAT = 0
# 		elif opt in ["--nosdss", "--noSDSS"]:
# 			print "Ignoring Sloan Digital Sky Survey..."
# 			doSDSS = 0
# 			doGetCoords = 0
# 		elif opt == "--usearchive":
# 			archive_module_list = [ module_list.shorthand_dict[optarg] ]
# 			if (optarg == "sdss"):
# 				doSDSS = 1
# 				if (not doCoordSearch): doGetCoords = 1
# 		elif opt in ["-c", "--coords"]:
# 			try:
# 				doCoordSearch = 1
# 				doGetCoords = 0
# 				coordsList = utils.ProcessCoords(optarg)
# 				targetName = "%s %s" % (coordsList[0], coordsList[1])
# 			except utils.CoordinateError, e:
# 				newmsg = "*** Problem with coordinate string \"%s\": ***\n" % optarg
# 				newmsg += str(e) + "\n"
...
# 				sys.exit(2)
# 		else:
# 			print "Unrecognized command-line option; try \"-h\" for a listing..."
# 			sys.exit(1)
	...
	theArchiveList = archive_list.ArchiveList( targetName, coordsList, searchBoxSize,
											 whichHSTarchive, options.useAAT, options.useSDSS,
											 archive_module_list )

	
	if (multipleSearches):
		targetLines = open(targetListFile).readlines()
		for currentLine in targetLines:
			...
			DoSearch( finalTargetName, theArchiveList, options.debugState, doThreading )
						


# 
# 				
# 		targetNameLines = open(targetListFile).readlines()
# 		for name in targetNameLines:
# 			if ( (name[0] == "#") or (name.strip() == "") ):
# 				# skip lines which begin with "#" or are empty
# 				continue
# 			targetName = name.strip()
# 			messageString = "Searching archives for %s" % targetName
# 			if doGetCoords:
# 				print "\n\nFetching coordinates for %s..." % targetName
# 				coordsList = FetchCoordinates(targetName)
# 				theArchiveList.InsertCoords(coordsList)
# 				messageString += " (%s, %s)" % (coordsList[0], coordsList[1])
# 			else:
# 				theArchiveList.InsertName(targetName)
# 			messageString = messageString + ", with search box = "
# 			messageString = messageString + "%4.1f arcmin..." % searchBoxSize
# 			print messageString
# 			DoSearch( targetName, theArchiveList, DEBUG, doThreading )
