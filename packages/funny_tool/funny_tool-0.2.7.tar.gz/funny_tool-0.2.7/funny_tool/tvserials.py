...
class TVSerials(object):
    ...
    '''
    def get_serials(self, serials_id):
        tvlist = get_list()
        if serials_id not in tvlist.keys():
            print('No TV serials for such a id.')
            return

        self.logfile = tvlist[serials_id] + '.txt'
        self.path = os.getcwd() + '/' + self.logfile
        print('TV : Starting to download ' + tvlist[serials_id])
        ...
        data = sess.get(self.prefix + 'resource/list/' + serials_id).text
