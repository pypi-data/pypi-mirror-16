...
class Algorithm(object):
    ...
    def _get_evals(self):
        ...
        for idx, (mix, arm) in enumerate(zip(self.evals_mixture, self.arms)):
            ...
            print 'self.history[%s]: ' % idx, self.history[idx]
            mix['1']['likelihoods'].append(get_log_likelihood(self.history[idx], means_of_arms[idx], sds_of_arms[idx]))
            mix['1']['populations'].append(len(self.history[idx]))
            mix['1']['likelihood'] = get_log_likelihood(self.history[idx], means_of_arms[idx], sds_of_arms[idx])
