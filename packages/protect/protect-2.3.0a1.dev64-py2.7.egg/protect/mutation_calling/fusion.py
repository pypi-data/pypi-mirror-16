...
def run_ericscript(job, fastqs, univ_options, ericscript_options):
    ...
    export_results(job, output_path, univ_options,
...
def parse_ericscript(infile):
    """
    Parses EricScript format and returns an Expando object with with basic features
    """
    ...
    eric_features = ['GeneName1', 'EnsemblGene1', 'strand1', 'chr1',
                     ...
                     'crossingreads', 'JunctionSequence']
...
def reformat_ericscript_output(job, fusion_file, univ_options):
    """
    Writes EricScript format in Transgene fusion format and returns a file store ID
    """
    job.fileStore.logToMaster('Reformatting EricScript ouput for %s' % univ_options['patient'])
    work_dir = job.fileStore.getLocalTempDir()
    input_files = {'results.tsv': fusion_file}
    input_files = get_files_from_filestore(job, input_files, work_dir, docker=False)
    output_path = os.path.join(work_dir, 'general_format.tsv')
    output_file = open(output_path, 'w')
    writer = csv.writer(output_file, delimiter='\t')
    header = ['#GeneName1', 'EnsemblGene1', 'strand1', 'chr1', 'Breakpoint1', 'DonorSeq',
              'GeneName2', 'EnsemblGene2', 'strand2', 'chr2', 'Breakpoint2', 'AcceptorSeq']
    writer.writerow(header)
    for record in parse_ericscript(open(input_files['results.tsv'], 'r')):
        five_prime_seq = ''.join(
            [base.upper() for base in record.JunctionSequence if base.islower()])
        three_prime_seq = ''.join([base for base in record.JunctionSequence if base.isupper()])
        writer.writerow([record.GeneName1, record.EnsemblGene1, record.strand1, record.chr1,
                         record.Breakpoint1, five_prime_seq,
                         record.GeneName2, record.EnsemblGene2, record.strand2, record.chr2,
                         record.Breakpoint2, three_prime_seq])
    output_file.close()
