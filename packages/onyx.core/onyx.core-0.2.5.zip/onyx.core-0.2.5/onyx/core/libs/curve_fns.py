...
#    # --- convert each curve's dates to a set
#    dates = [set(crv.dates) for crv in curves]
#
#    # --- find dates common to all curves
#    common_dates = set.intersection(*dates)
#
#    intersected_curves = []
#    for k, crv in enumerate(curves):
#        # --- get a list of missing dates for this curve
#        missing_dates = sorted(set.difference(dates[k], common_dates))
#        dts, idx = crv.dates, 0
#        mask = np.ones_like(dts, dtype=np.bool)
#        # --- look for each date that is not in the common set
#        for missing_date in missing_dates:
#            idx = bisect.bisect_left(dts, missing_date, idx)
#            if dts[idx] == missing_date:
#                mask[idx] = False
#        new_crv = crv.create_raw(dts[mask], crv.values[mask])
#        intersected_curves.append(new_crv)
