...
class GraphNode(object):
    ...
    def get_value(self):
        if self.valid:
            ...
            # --- before calculating the current value of a node clean-up the
            #     children set: this is needed to make sure that, once the node
            #     is invalidated, we don't keep references to spurious nodes.
            #     This is achieved removing this node from the set of parents
            #     of all its children
            node_id = self.get_id()
...
class CallableNode(GraphNode):
    __slots__ = ("VT", "obj_ref",
                 "parents", "children", "valid", "value", "args")
    ...
    def get_value(self, *args):
        ...
        if self.valid:
            ...
            self.value = getattr(self.obj_ref(), self.VT)(*self.args)
            self.valid = True
            return self.value
