...
""")
    parser.add_argument('--tmpfiles', '--tmpfiles-path',
                        metavar='PATH', dest='tmpfiles_path',
                        default=argparse.SUPPRESS,
                        help='change the folder that MiModD will use to store temporary files'
                        )
    parser.add_argument('--snpeff', '--snpeff-path',
                        metavar='PATH', dest='snpeff_path',
                        nargs='?',
                        default=argparse.SUPPRESS,
                        const=None,
                        help="set the location of the SnpEff installation to be used by MiModD. Omit PATH to indicate that SnpEff is not installed."
                        )
    parser.add_argument('-t', '--threads', '--multithreading-level',
                        dest='multithreading_level',
                        default=argparse.SUPPRESS,
                        help='change the maximal number of threads that any MiModD tool is allowed to use by default'
                        )
    parser.add_argument('-m', '--memory', '--max-memory',
                        dest='max_memory',
                        default=argparse.SUPPRESS,
                        help='change the maximal memory in GB that any MiModD tool is allowed to use by default (will not affect the snap tool)'
                        )
