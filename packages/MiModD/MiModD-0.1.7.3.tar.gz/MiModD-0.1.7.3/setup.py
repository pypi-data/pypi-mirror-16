...
class build_galaxy_data (Command):
    ...
    def run (self):
        # copy and modify Galaxy xml files
        if not os.path.isdir(GALAXY_TMP_DEST):
            os.mkdir(GALAXY_TMP_DEST)
        if not os.path.isdir(WRAPPERS_TMP_DEST):
            os.mkdir(WRAPPERS_TMP_DEST)

        shutil.copy(os.path.join(GALAXY_DATA, 'mimodd_tool_conf.xml'),
                    GALAXY_TMP_DEST)

        # create version-tagged copies of galaxy tool wrappers
        for wrapper in os.listdir(GALAXY_WRAPPERS):
            with open(os.path.join(GALAXY_WRAPPERS, wrapper),
                      'r', encoding='utf-8') as wrapper_in:
                first_line = wrapper_in.readline()
                if not 'version="x.x"' in first_line:
                    raise RuntimeError(
                        'Compromised tool identifier line '
                        'in Galaxy tool wrapper file {0}'
                        .format(wrapper)
                        )
            ...
            with open(os.path.join(WRAPPERS_TMP_DEST, wrapper),
