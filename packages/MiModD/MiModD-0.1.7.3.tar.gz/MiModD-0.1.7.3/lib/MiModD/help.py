...
help_commands = {
    'info': 'retrieve information about the samples encoded in a file for various supported formats',
    ...
    'reheader': 'from a BAM file generate a new file with specified header sections modified based on the header of a template SAM file',
    'sort': 'sort a SAM or BAM file by coordinates (or names) of the mapped reads',
    'snap': 'align sequence reads using the SNAP aligner',
    'snap-batch': 'run several snap jobs and pool the resulting alignments into a multi-sample SAM/BAM file',
    'snap-index': 'index a reference genome for use with the SNAP aligner',
    'varcall': 'predict SNPs and indels in one or more aligned read samples and calculate the coverage of every base in the reference genome using samtools/bcftools',
    'varextract': 'extract variant sites from BCF input as generated by varcall and report them in VCF',
    'covstats': 'summary coverage statistics for varcall output',
    'delcall': 'predict deletions in one or more samples of aligned paired-end reads based on coverage of the reference genome and on insert sizes',
    'vcf-filter': 'extract lines from a vcf variant file based on sample- and field-specific filters',
    'annotate': 'annotate a vcf variant file with information about the affected genes',
    'snpeff-genomes': 'list installed SnpEff genomes',
    'map': 'perform a linkage analysis between a selected phenotype and identified variants',
...
help_devtools = {'index': 'the MiModD.index tool',
                 'sanitize': 'the MiModD.sanitize tool',
                 }

# additional help topics with simple help strings and location of
# corresponding doc files
help_topics = {'configure': {'summary': 'Configuring MiModD',
                             'doc': None},
               'updates': {'summary': 'Keeping MiModD up to date',
                           'doc': None},
               'galaxy': {'summary': 'Preparing MiModD for use from Galaxy',
                          'doc': None},
               'fileformats': {'summary': 'Specifications of file formats supported by MiModD',
                               'doc': None},
               }
...
def help (args, parser):
    if args.topic is None:
        ...
        print_help_section(help_topics,
                           'other help topics', 'summary')
        ...
        print()
        print(textwrap.fill(
            'Note: You may need to replace "python3" in the above command with the name of the Python 3 interpreter that was used to install MiModD!'
            ))
        print()
        print()
        help_string = subprocess.check_output([sys.executable,
                                               '-m',
                                               'MiModD.{0}'.format(args.topic),
                                               '--help'],
        ...
        if doc_file is None:
            print('Our apologies, but this section of the help system needs yet to be written')
        else:
        ...
        # give up
        print('No help available for topic "{0}"'.
              format(args.topic)
              )
