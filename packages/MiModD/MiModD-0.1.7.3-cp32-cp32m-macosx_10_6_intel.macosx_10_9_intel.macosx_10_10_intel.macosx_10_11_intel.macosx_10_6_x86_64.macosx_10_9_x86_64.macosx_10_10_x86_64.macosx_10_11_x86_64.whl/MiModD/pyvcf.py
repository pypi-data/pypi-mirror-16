...
from . import pybcftools, bioobj_base
...
def parse_info (ifo):
    ...
    header_fields, sample_names = parse_header_line(line)
    return meta, header_fields, sample_names
...
class Info (object):
    ...
    @property
    def contigs (self):
        return [bioobj_base.Chromosome(contig_id,
                                       self.meta['contig'][contig_id]['length'])
                for contig_id in self.meta.get('contig', [])]
...
class VCFLikeFileReader (object):
...
class VCFReader (VCFLikeFileReader):
    ...
    def by_sample (self):
        excluded_gt_set = set('0/|.')
        if not self.ifo_isseekable:
            raise NotImplementedError ('by_sample method is currently implemented only for seekable files and file-like objects')
        if any(self.info.samples):
...
class VCFWriter (object):
    ...
    def __init__ (self, ofile_obj, header=None, samples=None):
        self.out = ofile_obj
        self.samples = samples
        if header is not None:
            if samples is not None:
                header = header.sample_slice(samples)
            self.out.write(str(header))
            self.out.write('\n')

    def write (self, record):
        w = 1 + self.out.write(self.record_to_str(record))
        self.out.write('\n')
        return w
