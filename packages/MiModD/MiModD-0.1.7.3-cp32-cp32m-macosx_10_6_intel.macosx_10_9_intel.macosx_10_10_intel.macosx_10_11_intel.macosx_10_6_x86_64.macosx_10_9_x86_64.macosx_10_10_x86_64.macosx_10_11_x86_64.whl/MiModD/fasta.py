...
def get_fasta_info (ifile):
    ...
    with open(ifile, 'r', encoding=FALLBACKENCODING) as ifo:
        for header, seq_iter in FastaReader(ifo):
            if header is None:
                raise FormatParseError('Expected ">" at start of file')
            seqlen = 0
            md5 = hashlib.md5()
            for seqline in seq_iter:
                seqline = seqline.strip().upper()
                if any(c not in IUPAC_DNA_codes for c in seqline):
                    raise FormatParseError('Invalid nucleotide code in sequence ' + header)
                seqlen += len(seqline)
                md5.update(seqline.encode())
            if seqlen == 0:
                raise FormatParseError('No nucleotide information found for sequence ' + header)
