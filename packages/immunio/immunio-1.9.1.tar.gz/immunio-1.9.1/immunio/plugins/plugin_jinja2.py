...
def add_hooks(run_hook, get_agent_func=None, timer=None):
    ...
    # Add environment cache hooks:
    add_environment_hooks(xss_disabled)
...
def add_environment_hooks(xss_disabled):
    """
    Monkeypatch the Jinja2 environment cache so we can tell the difference
    between our modified templates and the original templates when the
    XSS feature is enabled or disabled.

    Modify the template objects when they're inserted to record the xss
    setting at that time. Check that value when the item is loaded and
    return None if the mode has changed. This should result in Jinja2
    loading the template again, then storing that result in the cache.

    Jinja2 does have an `auto_reload` setting, and when disabled templates
    should not reloaded. This appears to be used for performace though, not
    to prevent loading changed files. Either way we ignore this setting
    by reloading the template again when the XSS setting is changed.

    The two objects below had to be created instead of just monkeypatching
    the cache instances because they use magic methods.
    """
    try:
    ...
    class XSSLruCache(jinja2.utils.LRUCache):
        def get(self, key, default=None):
            result = super(XSSLruCache, self).get(key, default)
            if result:
                xss_enabled = not xss_disabled()
                template_xss = getattr(result, '_immunio_xss', False)
                if xss_enabled != template_xss:
                    return None
            return result

        def __setitem__(self, key, value):
            xss_enabled = not xss_disabled()
            value._immunio_xss = xss_enabled
            super(XSSLruCache, self).__setitem__(key, value)

    class XSSDict(dict):
        def get(self, key, default=None):
            result = super(XSSDict, self).get(key, default)
            if result:
                xss_enabled = not xss_disabled()
                template_xss = getattr(result, '_immunio_xss', False)
                if xss_enabled != template_xss:
        ...
        def __setitem__(self, key, value):
            xss_enabled = not xss_disabled()
            value._immunio_xss = xss_enabled
            super(XSSDict, self).__setitem__(key, value)

    @monkeypatch(jinja2.environment, "create_cache")
    def _create_our_cache(orig, size):
        """
        Use the same create_cache values to create a cache that keeps
        track of if the templates cached were with XSS enabled or disabled.
        """
        if size == 0:
            return None
        if size < 0:
            return XSSDict()
        return XSSLruCache(size)

    @monkeypatch(jinja2.environment, "copy_cache")
    def _copy_our_cache(orig, cache):
        """
        Despite it's name, this doesn't actually copy the values, just the
        type. Modify it to know about our types.
        """
        if cache is None:
            return None
        elif isinstance(cache, XSSDict):
            return XSSDict()
        else:
            return XSSLruCache(cache.capacity)
...
def add_compiler_hooks(timer, xss_disabled):
    ...
    def _parse(orig, parser_self, *args, **kwargs):
        ...
        Because the Template always has the two extra fields, even when XSS
        is disabled, we do not want to ever skip this patch. Instead just
        set the values to None when XSS is disabled.
        """
        ...
        # If XSS is enabled, get the values from parser
        if not xss_disabled():
