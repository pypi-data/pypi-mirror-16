...
try:
    ...
    if retcode < 0:
        logging.error('%s, Child was terminated by signal: -%s' % (sys.stderr, retcode))
    elif retcode == 0:
        logging.debug('SUCCESS! %s, Child returned: %s' % (sys.stderr, retcode))
    else:
        logging.error('%s, Child returned: %s, see: http://tldp.org/LDP/abs/html/exitcodes.html'
                       % (sys.stderr, retcode))
except OSError as e:
    logging.error('%s, Execution failed: %s' % (sys.stderr, e))
