...
class AssetStore(object):
    ...
    def __init__(self):
        ...
        self._assets = {}
        self._module_names = []
        self.add_asset('reset.css', RESET.encode("utf-8"))
    ...
    def create_module_assets(self, module_name, css='', js=''):
        ...
        self._assets[fname + '.css'] = css_.encode("utf-8")
        self._assets[fname + '.js'] = js_.encode("utf-8")
...
class SessionAssets(object):
    ...
    def _load_asset_dynamically(self, fname, code=None):
        ...
        # Get code
        if code is None:
            code = self._store.load_asset(fname).decode("utf-8")
        if not code:
            return
        # Check
    ...
    def _get_js_and_css_assets(self, with_reset=False):
        ...
        if self._extra_model_classes:
            ...
            self.add_asset('index-extra-model-classes.css', css.encode("utf-8"))
            self.add_asset('index-extra-model-classes.js', js.encode("utf-8"))
        ...
        if with_reset:
            fname = 'reset.css'
            d[fname] = self._store.load_asset(fname).decode("utf-8")
...
def get_random_string(length=24, allowed_chars=None):
    ...
    try:
        ...
        random.seed(hashlib.sha256(bogus.encode("utf-8")).digest())

    return ''.join(srandom.choice(allowed_chars) for i in xrange(length))
