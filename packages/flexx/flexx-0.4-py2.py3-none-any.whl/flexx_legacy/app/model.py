...
class Model(with_metaclass(ModelMeta, event.HasEvents)):
    ...
    def __init__(self, session=None, is_app=False, **kwargs):
        ...
        Model._counter += 1
        self._id = self.__class__.__name__ + unicode(Model._counter)
        ...
        super(Model, self).__init__(_init_handlers=False, **kwargs)
    ...
    def _set_prop(self, name, value, _initial=False, fromjs=False):
        ...
        ischanged = super(Model, self)._set_prop(name, value, _initial)
    ...
    def emit(self, type, ev, fromjs=False):
        ev = super(Model, self).emit(type, ev)
    ...
    class JS(object):
        ...
        def _set_prop(self, name, value, _initial=False, frompy=False):
            ...
            if window.flexx.ws is None:  # Exported app
                super(JS, self)._set_prop(name, value, _initial)
                return
            
            ischanged = super(JS, self)._set_prop(name, value, _initial)
