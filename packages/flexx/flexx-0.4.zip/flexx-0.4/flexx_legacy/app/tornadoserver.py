            ...
            if not file_name:
                ...
                if '/' not in path:
                    ...
                    if session and session.status == session.STATUS.PENDING:
                        self.write(session.get_page().encode("utf-8"))
                    else:
                ...
                try:
                    ...
                    for i, line in enumerate(res.decode("utf-8").splitlines()):
                        table = {ord('&'): '&amp;', ord('<'): '&lt;', ord('>'): '&gt;'}
                ...
                try:
                    ...
                    super(MainHandler, self).write_error(404)
                else:
                    self.write(res)
