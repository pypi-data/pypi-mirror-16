...
class Config:
    ...
    def required_options(self):
        return {
            'facebook': [
                {'name': 'SENDER_ACTION',
                 'type': str,
                 'choices': ['mark_seen', 'typing_on', 'typing_off']},
    ...
    def check_required_config_options(self, config, required_options):
        ...
        for section in required_options:
            ...
            for option in required_options.get(section):
                ...
                if choices:
                    choice = self.get_parameter(section, option_name)
                    if choice not in choices:
                        self.raise_invalid_option_choice(section=section, choice=choice, choices=choices)
    ...
    @staticmethod
    def raise_invalid_option_choice(section, choice, choices):
        raise ValueError('Invalid choice in section:{section}. Valid choices: {valid_choices}, given: {choice}'.format(
            section=section, valid_choices=choices, choice=choice))
