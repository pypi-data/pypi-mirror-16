...
from messenger_sdk.templates import ThreadSettings
...
class FbManager:
    def __init__(self, api_client: ApiClient, config: Config):
        ...
        self._fb_thread_settings = config.get_parameter('facebook', 'FB_THREAD_SETTINGS')
    ...
    @gen.coroutine
    def set_thread_settings(self, settings: ThreadSettings):
        response = FbResponse(settings.__dict__())
        self.send(response=response, url=self._fb_thread_settings)
