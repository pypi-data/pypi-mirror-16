...
class FbManager:
    ...
    def send(self, response: FbResponse, url=None):
        ...
        print(type(response))
        print(isinstance(response, FbResponse))
        import pprint
        pprint.pprint(response)

        if not isinstance(response, FbResponse):
            raise TypeError('Invalid response class. Expected {expected}, {given} given.'.format(expected=FbResponse.__name__, given=type(response).__name__))
