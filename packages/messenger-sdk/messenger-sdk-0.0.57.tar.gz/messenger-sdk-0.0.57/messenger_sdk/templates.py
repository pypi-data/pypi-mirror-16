...
class ThreadSettings(FbObject, metaclass=ABCMeta):
...
class ButtonThreadSettings(ThreadSettings, metaclass=ABCMeta):
    NEW_THREAD = 'new_thread'
    EXISTING_THREAD = 'existing_thread'
    VALID_THREAD_STATES = [NEW_THREAD, EXISTING_THREAD]

    def __init__(self, setting_type, thread_state):
    ...
    def throw_if_not_valid(self):
        if self._setting_type not in self.VALID_THREAD_STATES:
            raise ValueError(
...
class GetStartedButton(ButtonThreadSettings):
    def __init__(self, call_to_action: StructuredPayload):
        super().__init__(setting_type=ThreadSettings.SETTING_TYPE_CALL_TO_ACTIONS, thread_state=self.NEW_THREAD)
    ...
    def throw_if_not_valid(self):
        if not isinstance(self._call_to_action, StructuredPayload):
            raise TypeError(
                'Invalid call_to_action type. Expected {expected}, given {given}.'.format(
                    expected=StructuredPayload.__class__.__name__,
                    given=type(self._call_to_action).__name__))
...
class PersistentMenu(ButtonThreadSettings):
    def __init__(self, call_to_actions: list, call_to_actions_limit=5):
        super().__init__(setting_type=ThreadSettings.SETTING_TYPE_CALL_TO_ACTIONS, thread_state=self.EXISTING_THREAD)
    ...
    def __dict__(self):
        template = {
            'call_to_actions': [{
                'payload': [call_to_action.__dict__() for call_to_action in self._call_to_actions]
            }]
        }
        template.update(super().__dict__())
