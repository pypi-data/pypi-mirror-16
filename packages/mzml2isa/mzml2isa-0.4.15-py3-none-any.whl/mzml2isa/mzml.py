...
if __name__ == '__main__':
    ...
    _ms = Ontology(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'psi-ms.obo'), False)
    _ims = Ontology(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'imagingMS.obo'), False)
    _ims.merge(_ms)

    if sys.argv[-1].endswith('.imzML'):
        print(imzMLmeta(sys.argv[-1], _ims).meta_json)
    else:
        ontology = Ontology(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'psi-ms.obo'))
        print(mzMLmeta(sys.argv[-1], _ms).meta_json)
