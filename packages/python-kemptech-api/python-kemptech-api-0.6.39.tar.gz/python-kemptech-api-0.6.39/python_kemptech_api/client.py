...
class HttpClient(object):
    ...
    def _do_request(self, http_method, rest_command,
        ...
        cmd_url = "{endpoint}{cmd}?".format(endpoint=self.endpoint,
                                            cmd=rest_command)
        # To be revisited - cmd_url contains credentials if Basic Auth
            ...
            if self.cert:
                ...
                url = ("https://{user}:{pw}@{ip}:{port}/progs/doconfig/"
                       "enableapi/set/yes").format(user=self.username,
                                                   ...
                                                   port=self.port)
                resp = self._tls_session.get(url, verify=False, timeout=1)
                url = ("https://{user}:{pw}@{ip}:{port}/progs/status/logout"
...
class VirtualService(HttpClient, KempBaseObjectModel):
    ...
    def __init__(self, loadmaster_info, ip_or_index, port=80, prot="tcp",
        ...
        self.real_servers = []
        self._waf = False
        self._waf_params = ['alertthreshold', 'intercept']
        self._esp = False
        self._esp_params = ['espenabled']
        if not is_sub_vs:
        ...
        try:
            if not update:
                ...
                if self._is_sub_vs:
                    ...
                    # Remove enable from the modvs request
                    self.enable = None
    ...
    def populate_default_attributes(self, service):
        ...
        rs_min = service.get('RsMinimum', 0)
        self.rsminimum = None if int(rs_min) == 0 else rs_min
...
class RealServer(HttpClient, KempBaseObjectModel):
    ...
    def __init__(self, loadmaster_virt_service_info, ip, port=80):
        ...
        # Needed to modify the RS port on LM
        self.newport = None
        try:
    ...
    def save(self, update=False):
        ...
        if is_successful(response):
            # If the rs port is to be changed set it to the new_port value
            if self.newport:
                self.rsport = self.newport
            response = self._get("/showrs", self._get_base_parameters())
            # Set new_port back to none as its only used when the rsport needs
            # to be changed on the LM
            self.newport = None
            data = get_data(response)
            rs_data = data.get("Rs", {})
