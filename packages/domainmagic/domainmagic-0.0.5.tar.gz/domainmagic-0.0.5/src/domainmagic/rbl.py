...
class RBLProviderBase(object):
    ...
    def __init__(self,rbldomain):
        ...
        self.logger=logging.getLogger('domainmagic.rbllookup.%s'%self.rbldomain)
...
class RPZProvider(RBLProviderBase):
    """lookups on rpz zones"""
    def _listed_identifiers(self, input, transform, dnsresult):
        listings = []
        listings.append((identifier, self.make_description(input=input, dnsresult=dnsresult, transform=transform,
                                                                   identifier=identifier, replycode=dnsresult)))
        return listings
