...
class Configuration:
    ...
    def __eq__(self, other):
        return (
            ...
            and
            self.joueurs == other.joueurs
            )

    def __lt__(self, other):
        if self == other:
            return (
                (self.mafieux, self.detectives, self.joueurs) < (other.mafieux, other.detectives, other.joueurs)
                )
...
def equilibre(intervalle_joueurs, intervalle_detectives):
    ...
    for joueurs in range(*intervalle_joueurs):
        configurations = [
            ...
            for mafieux, detectives in itertools.product(range(0, intervalle_joueurs[1]//2+1), range(*intervalle_detectives))
            if mafieux + detectives <= joueurs
            ]
        try:
            exces = min((conf for conf in configurations if conf.proba >= .5))
        except ValueError:
            exces = Configuration(joueurs, joueurs, 0)

        try:
            defaut = max((conf for conf in configurations if conf.proba <= .5))
        except ValueError:
            defaut = Configuration(joueurs, 0, joueurs)

        tableau.append([
            joueurs,
            "{:.1f}% ({}m, {}d)".format(100*defaut.proba, defaut.mafieux, defaut.detectives),
            "{:.1f}% ({}m, {}d)".format(100*exces.proba, exces.mafieux, exces.detectives),
        ])
...
def analyse():
    ...
    parser.add_argument(
        '-d', '--detectives',
        type=type_intervalle, default=[None, None],
...
def main():
    ...
    sys.setrecursionlimit(
            sys.getrecursionlimit() + 4 * arguments.players[1]
        )
