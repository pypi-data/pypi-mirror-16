...
def shoot():
    ...
    if options.demo_storage_baseline:
        ...
        db_factory = lambda : ZEO.DB(addr, ssl=client_ssl())
        ...
        headings = ('add', 'update', 'cached', 'read', 'loadB')
...
def run_test(db_factory, process_index, iqueue, oqueue,
    ...
    # cold, With db cache, but not object cache
    conn.cacheMinimize()
    for slot in slots:
        _ = slot.children
    ...
    try:
        ...
        # ZEO 4
        call = storage._server.rpc.call
    iqueue.get()
    start = time.time()
    for slot in slots:
        for it in range(transaction_size):
            call('loadBefore', slot.children[it]._p_oid, m64)
    cold_time = time.time() - start
    oqueue.put(time.time() - start)
