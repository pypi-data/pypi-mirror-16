...
class LunarLanderFactorized(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        # fire left, don't fire left, fire right, don't fire right, fire down, don't fire down
        self.action_space = spaces.Box(np.zeros(6), np.ones(6))
        # useful range is -1 .. +1
    ...
    def _reset(self):
        ...
        return self._step(np.zeros(6))[0]
    ...
    def _step(self, action):
        ...
        for i in range(3):
            assert action[2 * i] + action[2 * i + 1] <= 1, "%r (%s) invalid " % (action, type(action))
            assert action[2 * i] in [0, 1], "%r (%s) invalid " % (action, type(action))

        # Engines
        tip  = (math.sin(self.lander.angle), math.cos(self.lander.angle))
        side = (-tip[1], tip[0]);
        dispersion = [self.np_random.uniform(-1.0, +1.0) / SCALE for _ in range(2)]
        if action[4] == 1:  # Main engine
        ...
        if action[2] == 1:
            actions.append(3)
        for cur_action in actions:  # Orientation engines
            direction = cur_action-2
