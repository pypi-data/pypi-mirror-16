    ...
    try:
        with elasticsearch_config(env) as es_config:
            ...
            rows = [[mapping, ", ".join(fmt_interval(s, e) for s, e in intervals)]
                    for mapping, intervals in doc_types.iteritems()]
...
def get_doc_types(es_config, base_name):
    ...
    intervals_by_doc_type = {k: list(to_intervals(set(v))) for k, v in by_doc_type.iteritems()}
...
def main():
    ...
    try:
        ...
        logging.debug("%s", dict((k, v if k is not 'password' else '<???>') for k, v in vars(env).iteritems()))
