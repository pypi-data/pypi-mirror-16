...
class Centiment:
    ...
    def __init__(self):
        self._lexicon_intensifier = self.load_lexicon('lexicon/int_dictionary1.11.txt')
        self.PATTERN_INTENSIFIER = re.compile('|'.join(self.reversed_by(self._lexicon_intensifier)))
        self._replace_contraction = self.load_replace('lexicon/contractions.replace')
        self.PATTERN_CONTRACTION = re.compile(r'(' + '|'.join(
            [re.escape(key) for key in self.reversed_by(self._replace_contraction)]) + ')')
        self._list_negation = self.load_list('lexicon/negation.list')
        self.PATTERN_NEGATION = re.compile('|'.join(reversed(self._list_negation)))
        self._replace_prefix = self.load_replace('lexicon/prefix.replace')
    ...
    def parse_tweet(self, text):
        if isinstance(text, list):
            ...
            self._lexicon_emoticons = self.load_lexicon('lexicon/emoticons.lex', 5)
