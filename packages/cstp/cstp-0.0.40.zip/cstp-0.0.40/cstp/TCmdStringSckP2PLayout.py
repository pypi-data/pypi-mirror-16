...
def StartCmdStringSckP2PLayoutSH(dictPeerByParam, sHostName4Param, sSuffix, clsSckP2PLayout):
    dictParam = dictPeerByParam.get(sHostName4Param,{})
    if not dictParam:
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.get(%s)={}EXIT!' % sHostName4Param)
        sys.exit(-1)
    sPairId = dictParam.get('sPairId', '')
    if not sPairId:
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.sPairId=Null,EXIT!' % (sPairId))
        sys.exit(-1)
    sPairIdSH = 'SH_'+sPairId
    dictApp = dictPeerByParam.get(sPairIdSH,{})
    if not dictApp:
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.get(%s)={}EXIT!' % (sPairIdSH))
        sys.exit(-1)
    sIPPort = dictParam.get('sIPPort', '')
    sHubId = dictParam.get('sHubId', '')
    ...
    if sIPPort and sHubId and sSuffix and sAcctPwd:
        ...
        sErrMsg = '(%s) is null' % sErrMsg[:-1] #剔除最后一个逗号
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.sErrMsg=%s!EXIT!' % (sErrMsg))
        sys.exit(-1)
