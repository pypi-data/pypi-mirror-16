...
def StartCmdStringSckP2PLayoutSH(dictPeerByParam, sHostName4Param, sSuffix, clsSckP2PLayout):
    dictParam = dictPeerByParam.get(sHostName4Param,{})
    if not dictParam:
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.get(%s)={}' % sHostName4Param)
        sys.exit(-1)
    sIPPort = dictParam.get('sIPPort', '')
    sHubId = dictParam.get('sHubId', '')
    sPairId = dictParam.get('sPairId', '')
    dictApp = dictParam.get(sPairId,{})
    sAcctPwd = dictApp.get(sSuffix, '')
    sClientInfo = 'ShareHub'
    if sIPPort and sHubId and sPairId and sSuffix and sAcctPwd:
        cssa = clsSckP2PLayout(sHubId,sIPPort,sPairId,sSuffix,sAcctPwd,sClientInfo)
        cssa.StartMainThreadLoop()
    else:
        sErrMsg = ''
        ...
        sErrMsg = '(%s) is null' % sErrMsg[:-1] #剔除最后一个逗号
        PrintTimeMsg('StartCmdStringSckP2PLayoutSH.sErrMsg=%s!' % (sErrMsg))
        sys.exit(-1)
