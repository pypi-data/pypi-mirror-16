...
This module contains the :py:class:`Version` class.
"""
...
from jw.util.python3hell import Bytes, Str
...
class Version(object):
    ...
    _levelValues = {v: k for k, v in list(_levels.items())}
    ...
    def __str__(self):
        ...
        return ''.join(
            ...
            for p, l in zip([-10] + self.version, self.version)
        )
...
_DEFAULT_WITHIN_REGEX = r'version_*\s*=\s*["\'][^"\']*["\']'
...
def _Handle(filename, args, ops):
    ...
    newContent = bytearray(content, 'utf-8')
    for m in sorted(matches, key=lambda match: match.start(), reverse=True):
        ...
        if not args.list:
            ...
            if args.synchronize:
                ...
                if not args.dry_run:
                    newContent[m.start():m.end()] = Bytes(args.synchronizedVersion)
            else:
                if args.dry_run or args.verbose:
                    print('%s: %s --> %s' % (filename, content[m.start():m.end()], str(v)))
                if not args.dry_run:
                    newContent[m.start():m.end()] = Bytes(v)
        if args.list or getattr(args, 'hg_tag', False) is not False:
            vlist.append(v)
    # Write back changes
    outFilename = _TEMP_FILE_TEMPLATE % filename if args.all_or_nothing else filename
    open(outFilename, 'w').write(Str(newContent))
    result = 1
...
def _Main():
    ...
    # Check arguments
    if bool(args.move) + (bool(args.down) or bool(args.up)) + bool(getattr(args, 'hg_tag', False)) > 1:
    ...
    fileList = list(chain.from_iterable(glob.glob(g) for g in args.filename))
    if getattr(args, 'hg_tag', False) is not False:
        # Set Mercurial tag
        if len(fileList) != 1:
        ...
        # Prepare operations
        if args.move:
            ops = list(zip(([n for n in _OPERATIONS if n.startswith(o.lower())] for o in args.move), args.move))
