...
class KaptlUpdate:
    ...
    def update_project(self):
        if Utils.diff_two_strings(self.rules_old, self.rules) == '':
            ...
            self.app_name = self.common_methods.parse_rules(self.session, self.rules, self.stack, self.kaptl_cookie,self.license,self.recipe)
            if self.app_name is not None:
                self.file_info = self.common_methods.get_file_info(self.session, self.app_name, self.rules, self.stack,
                                                                   self.angular_only, self.app_id, self.license)
