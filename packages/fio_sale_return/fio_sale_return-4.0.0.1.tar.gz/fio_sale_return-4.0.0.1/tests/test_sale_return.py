...
class TestSaleReturn(unittest.TestCase):
    ...
    def test_0020_test_non_return_sale_line(self):
        ...
        self.assertEqual(
            ...
            self.sale_configuration.default_return_policy)
        sale_line.on_change_origin()
        self.assertIsNone(sale_line.return_policy)

        # Write a return policy on product
        self.product_template.return_policy = self.policy_2
        self.product_template.save()

        sale_line.product = self.product.id
        sale_line.on_change_product()
        self.assertEqual(
            sale_line.return_policy_at_sale,
            self.product_template.return_policy)
    ...
    def test_0030_test_return_sale_fulfilling_policy(self):
        ...
        # Create sale line
        return_sale_line = self.SaleLine(**{
            'sale': return_sale.id,
            'type': 'line',
            'quantity': -1,
            'product': self.product.id
        })
        return_sale_line.on_change_product()
        return_sale_line.on_change_quantity()
        return_sale_line.origin = '%s,%s' % (self.SaleLine.__name__, -1)

        return_sale_line.on_change_origin()
        self.assertIsNone(return_sale_line.return_policy)
        self.assertTrue(return_sale_line.is_return)

        return_sale_line.origin = \
            '%s,%s' % (self.SaleLine.__name__, sale_line.id)
        return_sale_line.on_change_origin()
        self.assertIsNotNone(return_sale_line.return_policy)
        self.assertEqual(
            return_sale_line.return_policy,
            sale_line.effective_return_policy_at_sale)

        return_sale_line.return_reason = self.reason_2.id
        return_sale_line.return_type = self.SaleLine.default_return_type()
        ...
        self.assertEqual(return_sale.invoices[0].type, 'out')
