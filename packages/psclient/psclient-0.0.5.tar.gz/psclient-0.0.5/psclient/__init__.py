...
def connect(
        ...
        name=None,
        config_file=None):
    """Loads a PredictiveService object from config files or environment
    variables.

    The PredictiveServer client needs a manager_endpoint, query_endpoint and
    admin_key to function at all. These can be specified directly, or they can
    be loaded from the environment variables or configuration files.
    ...
    query_endpoint: str, optional
        ...
        The query service endpoint. If not specified, then the manager will
        be queried for it.
...
def create(name, ec2_config, state_path, license_file_path = None, num_hosts = 1,
    ...
    state_path : str
        S3 path used to store state for the Predictive Service.

    license_file_path : str, optional
        The path to the local license file that will be used to launch the
        Predictive Service. If not specified, the default license file path
        is ~/.graphlab/config
    ...
    port : int, optional
        The administration port for the server, default is 8088. If you have firewall
        enabled in your enterprise, you need to open up this port in order to
        be able to access the administration port.
        The port value cannot be 9005, 9006, 19006 as they are reserved.

    admin_ui_port : int, optional
        The administration UI port for the server, default is 8888. If you have firewall
        enabled in your enterprise, you need to open up this port in order to
        be able to access the administration UI port.
        The port value cannot be 9005, 9006, 19006 as they are reserved.
