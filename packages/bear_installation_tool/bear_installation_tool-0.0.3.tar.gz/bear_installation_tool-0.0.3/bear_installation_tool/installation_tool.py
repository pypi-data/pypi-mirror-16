...
def check_failed_bears(bear_list):
    ...
    :return:          True if any bears failed installation, false if not.
    """
    if bear_list:
...
def install_bears(bear_names_list, install_deps):
    ...
    if check_failed_bears(bears_failed_list):
        sys.exit(1)
...
def main():
    ...
    if answer == "all":
        ...
        install_bears(sorted(get_all_bears_names_from_PyPI()), not args.ignore)
        return 0
        ...
        install_bears(answer, args.ignore)
        return 0
