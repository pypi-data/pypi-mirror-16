...
class LightingandPlugLoadEfficiency:
    ...
    def get_lighting_plugload_efficiency(self):
        ...
        # the lighting power in gbXML is always WattPerSqureMeter no matter if the unit is SI or IP, so now convert to WattPerSqureFeet here
...
class MonthlyHeating:
    def __init__(self, HeatingObject):
    ...
    def get_monthly_heating(self, thisHeatingObject):
        ...
        for eachValue in values:
            self.HeatingLoadResults.append(float(eachValue.text)/1000) # convert to kBTU
            
class MonthlyCooling:
    def __init__(self, CoolingObject):
        self.HeatingLoadResults = []
