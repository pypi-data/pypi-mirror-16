...
class LocalDeployment(object):
    ...
    def __init__(self, systemroot, localRepoPath, deployId, envAlias, packages, configs=None, logger=None):
    ...
    # 生成部署文件
    def deploy(self):
        deployEnvFile = os.path.join(self.currentDeployPath, 'env.properties')
        # 文件已经存在则不处理
        if os.path.isfile(deployEnvFile):
            self.logger.info('env file: %s already exist', deployEnvFile)
        envs = {}
        envs['APP_ROOT'] = self.currentDeployPath
        envs['APP_LOG_PATH'] = os.path.join(self.VAR_LOG_ROOT, self.envAlias)
        envs['APP_SHARE_PATH'] = os.path.join(self.VAR_SHARE_ROOT, self.envAlias)

        # 追加用户定义的环境变量
        if self.configs is not None:
            envs.update(self.configs)
...
def main(args):
    ...
    parser.add_argument('-e', '--envname', dest='envname', help="Environment name")
    ...
    deployment = LocalDeployment(systemroot, localRepoPath, deployId, alias, packages, configs)
