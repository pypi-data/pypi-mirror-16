        ...
        deployTemplateJsonStr = '''
{
    ...
    "localOverrides": [
        ...
        packages, configs = self._getAgentVersion(proxyaddr, port)
        ...
        deployTemplateJson['configs'].update(configs)
        ...
        if proxyHost is not None and proxyPort is not None:
            deployTemplateJson['configs']['PROXY_HOST'] = proxyHost
            deployTemplateJson['configs']['PROXY_PORT'] = proxyPort
        user = settings.getUser()
        if user is not None:
            deployTemplateJson['configs']['USER'] = user
        hostgroup = settings.getHostGroup()
        if hostgroup is not None:
            deployTemplateJson['configs']['HOSTGROUP'] = hostgroup
    ...
    def _getAgentVersion(self, proxyaddr, port):
        ...
        if r.status_code == 200:
            rjson = r.json()
            return rjson['packages'], rjson['configs']
        else:
            return None, None
