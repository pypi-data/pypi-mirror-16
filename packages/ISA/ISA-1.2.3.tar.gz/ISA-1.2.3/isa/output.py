...
class OutputHandler:
    ...
    def process(self, node, statistics):
        ...
        if self.output_type == 'standard_output':
            self.output_std(node, statistics)

    def output_std(self, node, statistics):
        """
        Output a node and its result to the standard output.

        :param node: node as specified by the process method.
        :param statistics: results as specified by the process method
        """
        with self.lock:
            print('-' * 80)
