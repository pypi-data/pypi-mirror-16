...
class Plugin:
    ...
    def install_package(self, server, package):
        ...
        _logger.info("[PLUGIN:%s] Install package [%s] if needed (using yum)" % (self.__class__.__name__, package))
        _logger.debug(command)
        _logger.debug(server.execute(command))
        command = "sudo apt-get install %s -y" % package
        _logger.info("[PLUGIN:%s] Install package [%s] if needed (using apt-get)" % (self.__class__.__name__, package))
        _logger.debug(command)
        _logger.debug(server.execute(command))
        _logger.info("[PLUGIN:%s] Installing package [%s] done" % (self.__class__.__name__, package))
