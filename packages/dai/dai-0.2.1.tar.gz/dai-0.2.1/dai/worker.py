...
class Task(object):
    ...
    def __setitem__(self, key, value):
        ...
        try:
            ...
            print('error occured during updating task.' + key)

    def __getattr__(self, attr):
        if attr in ['taskDoc', 'id', 'processor', 'subtasks', 'meteorClient', 'has_key']:
    ...
    def __setattr__(self, key, value):
        if key in ['taskDoc', 'id', 'processor', 'subtasks', 'meteorClient', 'has_key']:
...
class Widget(object):
    ...
    def __setitem__(self, key, value):
        ...
        try:
            self.meteorClient.call(
                'widgets.update', [self.id, {'$set': {key: value}}])
        except Exception as e:
            print('error occured during updating widget.' + key)
    ...
    def push(self, key, value):
        self.meteorClient.call(
            'widgets.update', [self.id, {'$push': {key: value}}])

    def pull(self, key, value):
        self.meteorClient.call(
            'widgets.update', [self.id, {'$pull': {key: value}}])

    def exec_widget_task_processor(self, task, widget, worker):
        import time
        id = widget._id
        widget_task_processor = self.default_task_processor
        ns = {'taskProcessors': taskProcessors, 'TASK': task, 'WIDGET': widget,
...
class Worker(object):
    ...
    def __setitem__(self, key, value):
        try:
            ...
            print('error occured during updating worker.' + key)
