...
class Results():
    ...
    def updateAll(self, Data):
        ...
        for i, d in enumerate(Data.Datasets):
            ...
            # Collect Datapoints of Blinks
            if self.blinkCorr:
                pca = PCA(n_components=10)
                pca.fit(tmpDataset)

                outliersPlus = ((np.transpose(pca.components_) - pca.components_.mean(axis=1)) > 4 * pca.components_.std(axis=1)).sum(axis=0)
                outliersMinus = ((np.transpose(pca.components_) - pca.components_.mean(axis=1)) < -4 * pca.components_.std(axis=1)).sum(axis=0)


                blinkComponentID = np.where(outliers==outliers.max())[0]
                blinkCurve
    ...
    def updateEpochs(self, Data):
        ...
        # Correct Epochs for Blink outliers
        argaerg
