...
class StatusAggregator(Process):
    ...
    def summarize(self, org_results):
        ...
        org_iteration_started = 1584946416000
        ...
        for app, app_data in org_results['apps'].iteritems():
            ...
            app_iteration_started = 1584946416000
            ...
            for collection, collection_data in app_data.get('collections', {}).iteritems():
                ...
                # APP
                if app_iteration_finished > collection_data.get('iteration_finished'):
                    app_iteration_finished = collection_data.get('iteration_finished')

                if app_iteration_started < collection_data.get('iteration_started'):
                    app_iteration_started = collection_data.get('iteration_started')

                if max_modified > collection_data.get('max_modified'):
                    max_modified = collection_data.get('max_modified')

                if min_modified < collection_data.get('min_modified'):
                    min_modified = collection_data.get('min_modified')

                if max_created > collection_data.get('max_created'):
                    max_created = collection_data.get('max_created')

                if min_created < collection_data.get('min_created'):
            ...
            if max_modified > org_results['summary']['max_modified']:
                org_results['summary']['max_modified'] = max_modified

            if min_modified < org_results['summary']['min_modified'] :
                org_results['summary']['min_modified'] = min_modified

            if max_created > org_results['summary']['max_created']:
                org_results['summary']['max_created'] = max_created

            if min_created < org_results['summary']['min_created']:
