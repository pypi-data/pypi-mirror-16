...
def init_logging(params, stdout_enabled=True):
    global logging_initialized

    root_logger = logging.getLogger()
    root_logger.handlers = []
    root_logger.setLevel(logging.getLevelName(params.get('log_level', 'INFO')))
    ...
    if stdout_enabled:
        stdout_logger.setLevel(logging.getLevelName(params.get('log_level', 'INFO')))

    log_file_name = os.path.join(params.get('log_dir'),
                             '%s-%s-%s-migrator.log' % (params.get('org'), params.get('migrate'), ecid))
    ...
    error_log_file_name = os.path.join(params.get('log_dir'), '%s-%s-%s-migrator-errors.log' % (
        params.get('org'), params.get('migrate'), ecid))
...
def run(self):
        ...
        while keep_going:
            ...
            try:
                ...
                status_aggregator_logger.info('Received status update for app/collection: [%s / %s]' % (app, collection))
                empty_count = 0
                org_results['summary'] = {
                    'max_created': -1,
                    'max_modified': -1,
