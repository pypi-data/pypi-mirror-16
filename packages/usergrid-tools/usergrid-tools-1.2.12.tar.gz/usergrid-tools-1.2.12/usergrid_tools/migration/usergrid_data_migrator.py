...
def russo_maneuver(app, collection_name, source_entity, attempts=0):
    response = reput(app, collection_name, source_entity, attempts)
    time.sleep(2.1)
    logger.info(source_entity.get('uuid'))
    response = migrate_graph(app, collection_name, source_entity, attempts) and response
...
def perform_migration(parameters):
    ...
    elif config.get('migrate') == 'russo_maneuver':
        operation = russo_maneuver
