...
def repair_user_role(app, collection_name, source_entity, attempts=0, depth=0):
    ...
    if ur_delete_name.is_success():
        logger.info('DELETE by Name for REPAIR was successful at URL %s' % target_entity_url_by_name)
    else:
    ...
    if ur_delete_by_uuid.is_success():
        logger.info('DELETE by UUID for REPAIR was successful at URL %s' % target_entity_url_by_uuid)
    else:
    ...
    if ur_delete_by_query.is_success() and ur_delete_by_query.first() is not None:
        logger.info('DELETE by QUERY for REPAIR was successful at URL %s' % url_delete_by_query)
    else:
