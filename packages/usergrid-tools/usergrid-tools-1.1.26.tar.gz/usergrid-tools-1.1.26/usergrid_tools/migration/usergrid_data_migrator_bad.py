...
logger = logging.getLogger('UsergridMigrator')
worker_logger = logging.getLogger('Worker')
collection_worker_logger = logging.getLogger('CollectionWorker')
error_logger = logging.getLogger('ErrorLogger')
audit_logger = logging.getLogger('AuditLogger')
stauts_aggregator_logger = logging.getLogger('StatusAggregator')
...
def log_queue_size(logger_, queue, label):
    if _platform == "linux" or _platform == "linux2":
        logger_.warn('CURRENT [%s] Queue Depth: %s' % (label, queue.qsize()))
    ...
    def run(self):
        ...
        while keep_going:
            ...
            try:
                ...
                stauts_aggregator_logger.debug(
                ...
                if app not in self.status['apps']:
                    self.status['apps'][app] = {
                        'collections': {}
                    }

                if collection not in self.status['apps'][app]['collections']:
                    self.status['apps'][app]['collections'][collection] = {}

                for collection, collection_data in status_map.iteritems():
                    self.status['apps'][app]['collections'][collection].update(collection_data)

                try:
                    for app, app_data in self.status['apps'].iteritems():
                        if app == 'ERROR':
                            continue
                        ...
                        for collection, collection_data in app_data.get('collections', {}).iteritems():
                            ...
                            try:
                                ...
                                self.status['summary']['count'] += collection_data['count']
                                self.status['summary']['bytes'] += collection_data['bytes']

                                # APP
                                if collection_data.get('iteration_started') < app_data['summary']['iteration_started']:
                                    app_data['summary']['iteration_started'] = collection_data.get('iteration_started')

                                if collection_data.get('iteration_finished') > app_data['summary'][
                                    'iteration_finished']:
                                    app_data['summary']['iteration_finished'] = collection_data.get(
                                        'iteration_finished')
                                ...
                                # ORG
                                if collection_data.get('iteration_started') < self.status['summary'][
                                    'iteration_started']:
                                    self.status['summary']['iteration_started'] = collection_data.get(
                                        'iteration_started')

                                if collection_data.get('iteration_finished') > self.status['summary'][
                                    'iteration_finished']:
                                    self.status['summary']['iteration_finished'] = collection_data.get(
                                        'iteration_finished')

                                if collection_data.get('max_modified') > self.status['summary']['max_modified']:
                                    self.status['summary']['max_modified'] = collection_data.get('max_modified')

                                if collection_data.get('min_modified') < self.status['summary']['min_modified']:
                                    self.status['summary']['min_modified'] = collection_data.get('min_modified')

                                if collection_data.get('max_created') > self.status['summary']['max_created']:
                                    self.status['summary']['max_created'] = collection_data.get('max_created')

                                if collection_data.get('min_created') < self.status['summary']['min_created']:
                                    self.status['summary']['min_created'] = collection_data.get('min_created')
                            except:
                                print('foo')

                        log_queue_size(stauts_aggregator_logger, self.worker_queue, 'Entity Queue')

                        stauts_aggregator_logger.warn('UPDATED status of org processed: %s' % json.dumps(self.status))
                ...
                stauts_aggregator_logger.warn('FINAL status of org processed: %s' % json.dumps(self.status))
                self.final_status_queue.put(self.status)
                raise e

            except Empty:
                log_queue_size(stauts_aggregator_logger, self.worker_queue, 'EntityQueue')

                self.final_status_queue.put(self.status)
                stauts_aggregator_logger.warn('CURRENT status of org processed: %s' % json.dumps(self.status))

                stauts_aggregator_logger.warning('EMPTY! Count=%s' % empty_count)
        ...
        logger.warn('FINAL status of org processed: %s' % json.dumps(self.status))
        self.final_status_queue.put(self.status)
...
class EntityWorker(Process):
    def __init__(self, entity_queue, handler_function, response_queue):
    ...
    def run(self):
        ...
        while keep_going:
            ...
            try:
                ...
                app, collection_name, entity = self.entity_queue.get(timeout=60)
                empty_count = 0
                ...
                if self.handler_function is not None:
                    try:
                        ...
                        if processed:
                            ...
                            failed_uuids.append(entity.get('uuid'))
...
def process_edges(app, collection_name, source_entity, edge_name, connection_stack):
    ...
    while len(connection_stack) > 0:
        ...
        result = create_connection(app, collection_name, source_entity, edge_name, target_entity)
...
def migrate_data(app, collection_name, source_entity, attempts=0, force=False):
    ...
    try:
        ...
        logger.error('error in migrate_data on entity: %s' % json.dumps(source_entity), exc_info=True)

    logger.error(
        'UNSUCCESSFUL migrate_data | success=[%s] | attempts=[%s] | entity=[%s / %s / %s] | created=[%s] | modified=[%s]' % (
            False, attempts, config.get('org'), app, source_identifier, source_entity.get('created'),
            source_entity.get('modified'),))
...
def do_operation(apps_and_collections, operation):
    ...
    if _platform == "linux" or _platform == "linux2":
        entity_queue = JoinableQueue(maxsize=config.get('queue_size_max'))
        collection_queue = JoinableQueue(maxsize=config.get('queue_size_max'))
        status_queue = JoinableQueue(maxsize=config.get('queue_size_max'))
        final_status_queue = JoinableQueue(maxsize=config.get('queue_size_max'))
    else:
        entity_queue = JoinableQueue()
        collection_queue = JoinableQueue()
        status_queue = JoinableQueue()
        final_status_queue = JoinableQueue()
    ...
    try:
        ...
        for app, app_data in apps_and_collections.get('apps', {}).iteritems():
            ...
            for collection_name in app_data.get('collections'):
                ...
                log_queue_size(logger, collection_queue, 'Collection Queue')
        ...
        if collection_count > 0:
            ...
            logger.info('Joining Collection queue...')
            collection_queue.join()

            logger.info('Joining Collection workers...')
            # allow collection workers to finish using join, not wait_for
            [w.join() for w in collection_workers if w.is_alive()]
            logger.info('Collection workers done!')

            log_queue_size(logger, entity_queue, 'Entity Queue')

            logger.info('Joining Entity workers...')
            # allow entity workers to finish using join, not wait_for
            [w.join() for w in entity_workers if w.is_alive()]

            logger.info('Entity workers done!')
    ...
    while True:
        try:
            status = final_status_queue.get(timeout=1)
...
def perform_migration_external(parameters):
    return perform_migration(parameters)


def perform_migration(parameters):
    global config, cache, ecid, session_source, session_target, queue
    ...
    if config.get('migrate') == 'graph':
        ...
        logger.warn('Since permissions migration was specified, overwriting included collections to be %s...' % config[
            'collection'])
