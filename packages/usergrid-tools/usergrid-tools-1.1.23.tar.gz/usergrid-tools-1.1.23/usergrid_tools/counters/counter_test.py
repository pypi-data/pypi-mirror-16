...
counter_increment_url_tempalte = '{host}/{org}/{app}/events'
counter_retrieve_url_tempalte = '{host}/{org}/{app}/counters?counter={counter}'
...
def test_counter(host, org, app, counter, inc_value=1, sleep_time=30, timestamp=None):
    start_value = check_counter_value(host, org, app, counter)
    ...
    while later_value <= start_value:
        ...
        later_value = check_counter_value(host, org, app, counter)
...
def check_counter_value(host, org, app, counter):
...
def parse_args():
    ...
    parser.add_argument('--host',
                        help='The host URL to use for testing, following the pattern: {host}/{org}/{app}',
                        type=str,
                        required=True)
