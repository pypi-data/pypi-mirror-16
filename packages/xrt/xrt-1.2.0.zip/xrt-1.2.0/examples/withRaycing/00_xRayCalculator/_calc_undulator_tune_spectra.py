...
source = rs.Undulator(eE=3.0, eI=0.5, eEpsilonX=0.263, eEpsilonZ=0.008,
                      ...
                      eEspread=0*8e-4, distE='BW', byParts=True)

energy = np.linspace(2000, 30000, 2801)
theta = np.linspace(-1, 1, 21) * 30e-6  # 60 µrad opening
psi = np.linspace(-1, 1, 21) * 30e-6  # 60 µrad opening
xlims, ylims = (3, 24), (1e14, 5e15)
Ks = np.linspace(0.12, 1.92, 19)
Kmax, gmin = 1.92, 4.2  # needed to calculate gap(K)
gaps = np.log(Kmax/Ks) / np.pi * source.L0 + gmin
harmonics = range(1, 12, 2)  # odd harmonics
...
for harmonic, color in zip(harmonics, colors):
    fname = 'tmp.d0{0}'.format(harmonic)
    spectraE, spectraK, spectraF = np.loadtxt(
        fname, skiprows=10, usecols=(1, 2, 3), unpack=True)
    plt.loglog(spectraE/1000, spectraF, 'o--', color=color, lw=2)
