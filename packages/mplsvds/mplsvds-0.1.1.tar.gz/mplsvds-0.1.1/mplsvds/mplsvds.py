...
# note that ticksizes are 0 for sns whitegrid style
...
# set the matplotlib rcparams to use sns whitegrid with svds custom style
sns.set_style('whitegrid', svds_style)
...
svds_color_dict = {v: k for k, v in hex_to_name.iteritems()}
...
def hex2rgb(colors):
    ...
    rgb_colors = zip(
        *[
          [int(c[i:i+n], 16) / float(255) for c in colors]
