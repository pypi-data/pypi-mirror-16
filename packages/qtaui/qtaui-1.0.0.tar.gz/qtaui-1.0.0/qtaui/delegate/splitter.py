...
class QAUISplitterDelegate(QAUIDelegate):
    ...
    def saveState(self):
        state = {'type': 'splitter', 'orientation': 0 if self._orientation == QtCore.Qt.Horizontal else 1, 'ratios': self._layout._ratios}
    ...
    def addChild(self, child, position=None):
        if position is None:
            position = QAUIPos.RIGHT if self._orientation == QtCore.Qt.Horizontal else QAUIPos.BOTTOM
