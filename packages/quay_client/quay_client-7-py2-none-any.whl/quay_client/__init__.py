...
class QuayClient(object):
    ...
    def __init__(self, **kwargs):
        ...
        """
        self.host = os.getenv("QUAY_HOST", kwargs.get("host", "https://quay.io"))
    ...
    def get_tags(self, artifact, **kwargs):
        ...
        Returns:
            list: Tags list or None if the repository was not found
        """
        ...
        if limit > 100:
            ...
            for _ in xrange(1, cycle + 1):
                ...
                latest_tags.update([tag["name"] for tag in history_response["tags"]])
                params["page"] += 1
            ...
            return list({tag["name"] for tag in history_response["tags"]}) if history_response else None
    ...
    def set_tag_on_existing(self, artifact, new_tag, existing_tag):
        ...
        return self.set_tag(artifact, new_tag, latest_tags[0])
    ...
    def find_trigger(self, artifact, service="github"):
        ...
        for trigger in list_triggers["triggers"]:
            if trigger["service"] == service and trigger["is_active"]:
                return trigger["id"]
    ...
    def start_build_trigger(self, artifact, branch_name, trigger_id):
        ...
        logger.info("Build: {}/repository/{}/build/{}".format(self.host, repository, build_uuid))
        return build_uuid
    ...
    def start_build_url(self, artifact, branch_name, url, **kwargs):
        ...
        Keyword Args:
            ...
            extra_tags (list): If you want to add other tags than the branch_name (eg: extra_tags= [42, "foobar"])
        ...
        artifact_tags = [branch_name]

        if "extra_tags" in kwargs:
            artifact_tags.extend(kwargs.get("extra_tags"))

        body = QuayClient._params_filter({
            "docker_tags": artifact_tags,
    ...
    def wait_build_complete(self, artifact, build_uuid, build_timeout=30 * 60, refresh_interval=5):
        ...
        while True:
            ...
            if build_status["phase"] == "error":
                raise QuayError("Build failed: {}/repository/{}/build/{}".format(self.host,
                                                                                 repository,
                                                                                 build_uuid))
            ...
            if build_status["phase"] != current_phase:
                ...
                logger.info("Reset timer before timeout".format(current_phase))
            ...
            logger.info("Build time: {}/{}".format(build_current_time, build_timeout))
    ...
    def wait_security_scan_complete(self, artifact, image_id,
        ...
        while True:
            ...
            if scan_current_time > scan_timeout:
                raise QuayError("Scan timeout: {}/repository/{}/image/{}?tab=vulnerabilities"
