...
class Build(DistutilsBuild):
    ...
    def check_version(self):
        ...
        try:
            ...
            raise BuildError("""Unable to execute '{}'. HINT: are you sure `go` is installed?
...
(DETAIL: original error: {}.)""".format(' '.join(cmd), e))
            ...
            if 'buildmode' not in build_help:
                raise RuntimeError("""Your Go installation looks too old: go_vncdriver requires at least Go 1.5. Fortunately, go_vncdriver is an optional (though significant) performance enhancement, and you can still use gym-vnc without it.
