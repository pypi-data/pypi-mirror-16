...
from pyfeld.tkui import RaumfeldDesktop
...
def get_specific_zoneinfo(uc, format):
    ...
    if format == 'json':
        ...
        if 'TrackMetaData' in results:
            result += get_didl_extract(results['TrackMetaData'], format)
        result += '}'
        return result
    else:
...
def run_main():
    ...
    if operation == 'ui':
        ...
        if device_format == 'udn':
            ...
            udn = get_room_udn(argv[argpos])
        location = get_device_location_by_udn(udn)
