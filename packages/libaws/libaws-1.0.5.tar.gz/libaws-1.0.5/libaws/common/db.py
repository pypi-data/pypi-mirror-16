...
class Ec2InstanceDb(BaseDb):
    ...
    def save_config(self,image_id,keyname,security_group_id,subnet_id,pem_file_path,vpc_id):

        self.delete_all_network_config()
        region = utils.get_region()
        delete_sql = '''
            delete from instance where region="%s"
        ''' % (region,)
        self.delete(delete_sql)

        insert_sql = '''
            insert into instance(keyname,subnet_id,image_id,security_group_id,key_path,vpc_id,region) values(?,?,?,?,?,?,?)
        '''
        region = utils.get_region()
        data = [(keyname,subnet_id,image_id,security_group_id,pem_file_path,vpc_id,region),]
