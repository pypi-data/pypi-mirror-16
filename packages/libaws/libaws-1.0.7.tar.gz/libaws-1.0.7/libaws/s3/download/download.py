...
def download_key_parts(s3_file_obj,download_file_obj,f,callback=None):
    '''
        下载文件分块
    '''
    ...
    for row in results:
        ...
        logger.debug('part_id:%d,part_size:%ld,read_size:%ld,total download size:%ld,file postion:%ld',range_id,file_range.size,read_size,total_download_size,f.tell())
...
def download_file(download_config):
    ...
    if config.LOGGER_DISABLED or not download_config.enable_debug_log:
        with callback.DownloadProgressPercentage(download_file_obj.path,download_file_obj.size,download_size) as cb:
            download_key_parts(s3_file_obj,download_file_obj,f,cb)
    else:
        download_key_parts(s3_file_obj,download_file_obj,f)
    f.close()
