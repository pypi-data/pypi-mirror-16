...
class MultiPartDownloader(object):
    ...
    def download(self,body,file_obj,callback=None):
        ...
        for chunk in iter(lambda:body.read(block_size),b''):
            ...
            logger.info('file:%s download percentage is %s ====================================download size(%ld/%ld)',file_path,percent,download_size,self.download_file_obj.size)
            logger.debug('download part:%d start:%ld,block_size:%ld',self.file_range.range_id,start_byte,block_size)
            #下载文件内容完成
