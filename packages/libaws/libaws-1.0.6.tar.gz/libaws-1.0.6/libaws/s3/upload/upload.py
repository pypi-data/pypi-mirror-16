...
def upload_file(upload_config):
    ...
    if config.LOGGER_DISABLED or not upload_config.enable_debug_log:
        ...
        multi_part_upload,part_num,part_ids = upload_file_multi_parts(file_obj,callback=callback.UploadProgressPercentage(file_path,upload_size))
    else:
    ...
    if complete_multi_parts(file_obj,multi_part_upload,part_ids,part_num):
        logger.info("file:%s upload to bucket %s success,elapse time %s",file_path,bucket,s3_upload_db.get_upload_time(upload_id))
    else:
        logger.error("file:%s upload to bucket %s error",file_path,bucket)
