...
class PaymentTypeAdminForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['payment_hook'].widget = forms.Select(choices=get_model_choices())


class DeliveryTypeAdminForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super(DeliveryTypeAdminForm, self).__init__(*args, **kwargs)
        self.fields['pricing_hook'].widget = forms.Select(choices=get_model_choices())
    ...
    def clean(self):
        ...
        if data['pricing_method'] in (DeliveryType.METHOD_PERCENT,
                                      DeliveryType.METHOD_CONSTANT):
            if not data.get('price'):
                raise ValidationError(
                    _('You need to enter price')
                )
        elif data['pricing_method'] == DeliveryType.METHOD_API:
            if data.get('price'):
                raise ValidationError(
        ...
        elif data['pricing_method'] == DeliveryType.METHOD_FREE:
            if data.get('price'):
                raise ValidationError(
                    _('You should not enter price')
                )
