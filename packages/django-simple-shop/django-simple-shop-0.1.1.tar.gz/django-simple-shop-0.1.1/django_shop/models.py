...
class ProductCategory(models.Model):
    ...
    def products_ordered(self):
        """Получить упорядоченный по убыванию доступного остатка список товара в категории.

        В список попадают только те товары, которые есть в наличии или помечены как
        отображающиеся всегда.

        :rtype: list[Product]
        """
        qs = self.products.annotate(Sum('entries__quantity')).filter(
            Q(always_visible=True) | Q(entries__quantity__sum__gt=0))
        return sorted(qs, key=lambda x: x.entries__quantity__sum or 0, reverse=True)
    ...
    def have_to_display(self):
        ...
        return self.products.annotate(Sum('entries__quantity')).filter(
            Q(always_visible=True) | Q(entries__quantity__sum__gt=0)).exists()
...
class Product(models.Model):
    ...
    weight = models.IntegerField(verbose_name='Вес', help_text='Масса единицы товара в упаковке (брутто) в граммах, '
    ...
    always_visible = models.BooleanField('Виден всегда', default=False,
                                         help_text='Включите, если товар должен отображаться, даже если его нет в '
                                                   'наличии')
...
class ProductEntry(models.Model):
    ...
    quantity = models.DecimalField('Количество', max_digits=9, decimal_places=2)
...
class DeliveryType(models.Model):
    ...
    ADDRESS_SELECT = (
        ...
        (SELECT_LOCAL, 'Ввод адреса в Петербурге'),
        (SELECT_METRO, 'Ввод станции метро и контактов'),
    ...
    delivery_hook = models.CharField(
        verbose_name='Метод определения цены', max_length=30,
        help_text='Используется при указании цены для API', blank=True
    )
    ...
    def products_string(self):
        ...
        for product in self._products:
            if product.product.is_weight:
                ...
                measure = 'шт'
            products_data.append(' - {} ({:.1} {});'.format(product.product.name, product.quantity, measure))
        return '\n'.join(products_data)
...
class ProductOrder(models.Model):
    ...
    def reduce_stock_amount(self):
        ...
        if not self.product.is_unlimited:
            qs = self.product.entries.filter(quantity__gt=0).select_for_update()
            stock_amount = qs.aggregate(Sum('quantity')).get('quantity__sum') or 0
