...
class ProductCategoryManager(models.Manager):
    def get_available(self):
        return self.annotate(
            product_amount=Sum('products__entries__quantity')).filter(
                Q(products__is_unlimited=True) |
                Q(products__always_visible=True) |
                Q(product_amount__gt=0)
        )
...
class ProductCategory(PriorityModel):
    ...
    image = models.ImageField('Изображение', upload_to='categories/%Y/',
...
class ProductManager(models.Manager):
    ...
    def get_by_value(self, fields_values):
        q = Q()
...
class Product(models.Model):
    ...
    def check_fields(self):
        ...
        with transaction.atomic():
            fields_to_have = ProductCategoryCustomField.objects.filter(category__in=categories)
            current_fields = ProductCategoryCustomField.objects.filter(values__product=self)
            for field in fields_to_have:
                if field not in current_fields:
                    ProductCustomFieldValue.objects.create(product=self, field=field)
