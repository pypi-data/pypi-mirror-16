...
class BasketModifyForm(forms.Form):
    ...
    amount = forms.DecimalField(min_value=0.1, required=False, decimal_places=1)
    ...
    def clean(self):
        data = super(BasketModifyForm, self).clean()
...
class ProductOrderForm(forms.Form):
    ...
    product = forms.ModelChoiceField(queryset=Product.objects.all(),
                                     widget=forms.HiddenInput)
    amount = forms.DecimalField(min_value=0.1, decimal_places=1, required=False,
                                widget=forms.HiddenInput)
