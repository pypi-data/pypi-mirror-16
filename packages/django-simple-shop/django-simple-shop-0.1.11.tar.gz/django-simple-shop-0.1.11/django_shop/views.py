...
class ProductListView(ProductCategoryFilterMixin, ListView):
    ...
    def _extract_filters(self):
        ...
        fields = self.category.fields.filter(
            is_filter=True, key__in=self.request.GET.keys())
        filters = {}
        for field in fields:
            value = self.request.GET[field.key]
            if field.choices and value not in field.choices_set:
                continue
            filters[field] = field.convert_from_request(value)
        return filters
    ...
    def get_queryset(self):
        ...
        if filters:
            qs = self.model.objects.get_by_value(filters, qs)
        return qs
...
class ProductDetailView(ProductCategoryFilterMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        return qs.select_related('custom_fields')
...
class CheckoutFormView(TemplateView):
    ...
    def post(self, request, *_, **kwargs):
        ...
        try:
            ...
            logger.exceprion('Unexpected error on order creation')
            order_form.add_error(
