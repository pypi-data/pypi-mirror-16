...
from ..models import ProductCategory, Product, ProductEntry, ProductImage, DeliveryAddress, DeliveryType, Order, \
    ProductOrder, PaymentType
...
class ProductTestCase(TestCase):
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name='Продукт', slug='product', category=self.category, price=1000,
            weight=1500, short_description='Короткое описание', description='Описание'
        )

    def test_empty_product(self):
        self.assertEqual(self.product.in_stock, 0)
        self.assertEqual(self.product.primary_image, 'placeholder.jpg')

    def test_has_stock_entries(self):
        ProductEntry.objects.create(product=self.product, price=1000, quantity=4.5)
        ProductEntry.objects.create(product=self.product, price=1000, quantity=2)
        self.assertEqual(self.product.in_stock, Decimal('6.5'))

    def test_has_stock_and_order_entries(self):
        ProductEntry.objects.create(product=self.product, price=1000, quantity=4.5)
        ProductEntry.objects.create(product=self.product, price=1000, quantity=2)
        dt = DeliveryType.objects.create(name='Доставка', pricing_method=delivery.METHOD_FREE)
        da = DeliveryAddress.objects.create(name='Имя', email='test@test.com', phone='+79991234567')
        order = Order.objects.create(sid='12345', delivery_type=dt, delivery_address=da, delivery_price=0)
        ProductOrder.objects.create(product=self.product, order=order, quantity=5.1, price=5000)
    ...
    def test_has_stock_and_more_order_entries(self):
        ...
        da = DeliveryAddress.objects.create(name='Имя', email='test@test.com', phone='+79991234567')
        order = Order.objects.create(sid='12345', delivery_type=dt, delivery_address=da, delivery_price=0)
        ProductOrder.objects.create(product=self.product, order=order, quantity=6.7, price=5000)
        self.assertEqual(self.product.in_stock, 0)

    def test_has_images(self):
        ProductImage.objects.create(product=self.product, image='/image1.png', priority=1)
        ProductImage.objects.create(product=self.product, image='/image2.png', priority=2)
        ProductImage.objects.create(product=self.product, image='/image18.png', priority=18)
        self.assertEqual(self.product.primary_image, '/image1.png')
...
class ProductCategoryTestCase(TestCase):
    ...
    def test_empty(self):
        self.assertEqual(self.category.products_ordered(), [])
        self.assertFalse(self.category.has_products)
        self.assertEqual(self.category.has_in_stock, 0)
    ...
    def test_product_with_amount(self):
        ...
        p2 = Product.objects.create(
            name='Продукт2', slug='product2', category=self.category, price=1000,
            weight=1500, short_description='Короткое описание', description='Описание'
        )
        ProductEntry.objects.create(product=p2, price=1000, quantity=3.4)
        p3 = Product.objects.create(
            name='Продукт3', slug='product3', category=self.category, price=1000,
            weight=1500, short_description='Короткое описание', description='Описание',
            always_visible=True
        )

        self.assertEqual(self.category.products_ordered(), [p1, p2, p3])
        self.assertTrue(self.category.has_products)
        self.assertEqual(self.category.has_in_stock, Decimal('7.9'))


class PaymentTypeTestCase(TestCase):
    def test_type_online(self):
        pt = PaymentType.objects.create(name='Кеш', payment_type=PaymentType.TYPE_OFFLINE)
    ...
    def test_type_offline(self, m_manager):
        ...
        pt = PaymentType.objects.create(name='Кеш', payment_type=PaymentType.TYPE_ONLINE)
...
class DeliveryTypeTestCase(TestCase):
    def setUp(self):
        self.af = {'name': 'Имя', 'email': 'test@test.com', 'phone': '+79991234567'}
    ...
    def test_method_free(self):
        ...
        self.assertEqual(
            dt.get_price_and_address('', self.af, Decimal(1234), 1500, (30, 30, 40)),
            (Decimal(0), self.af)
        )
    ...
    def test_method_api(self, m_del):
        ...
        m_del.return_value = t(Decimal('12367'), {'id': 12345, 'address': 'Адрес'})
        ...
        self.assertEqual(
            dt.get_price_and_address(json.dumps({'id': 12345, 'address': 'Адрес'}),
                                     self.af, Decimal(1234), 1500, (30, 30, 40)),
            (Decimal('12367'), {'id': 12345, 'address': 'Адрес'})
        )
        m_del.assert_called_once_with(
            '', {'options': {'address': 'Адрес', 'type_price': 10, 'hook': '', 'id': 12345},
                 'address': {'phone': '+79991234567', 'name': 'Имя', 'email': 'test@test.com'},
                 'order_price': Decimal('1234'), 'weight': 1500, 'measures': (30, 30, 40)}
        )
...
class ProductOrderTestCase(TestCase):
    def setUp(self):
        ...
        dt = DeliveryType.objects.create(name='Доставка', pricing_method=delivery.METHOD_FREE)
        da = DeliveryAddress.objects.create(name='Имя', email='test@test.com', phone='+79991234567')
        self.order = Order.objects.create(sid='12345', delivery_type=dt, delivery_address=da, delivery_price=0)

    def test_reduce_stock_amount(self):
        po = ProductOrder.objects.create(product=self.product, order=self.order, quantity=1, price=1000)
        pe = ProductEntry.objects.create(product=self.product, price=1000, quantity=4.5)
    ...
    def test_reduce_stock_amount_from_two_entries(self):
        po = ProductOrder.objects.create(product=self.product, order=self.order, quantity=3, price=1000)
        pe1 = ProductEntry.objects.create(product=self.product, price=1000, quantity=1.5)
        pe2 = ProductEntry.objects.create(product=self.product, price=1000, quantity=2.5)
        po.reduce_stock_amount()
        self.assertEqual(po.stock_reduced, True)
    ...
    def test_not_enough(self):
        po = ProductOrder.objects.create(product=self.product, order=self.order, quantity=3, price=1000)
        pe = ProductEntry.objects.create(product=self.product, price=1000, quantity=1.5)
...
class OrderTestCase(TestCase):
    def setUp(self):
        ...
        self.pe1 = ProductEntry.objects.create(product=self.product, price=1000, quantity=1.5)
        self.pe2 = ProductEntry.objects.create(product=self.product, price=1000, quantity=2.5)

        self.dt = DeliveryType.objects.create(name='Доставка', pricing_method=delivery.METHOD_CONSTANT, price=126)
        da = DeliveryAddress.objects.create(name='Имя', email='test@test.com', phone='+79991234567')
        self.order = Order.objects.create(sid='12345', delivery_type=self.dt, delivery_address=da, delivery_price=126)

        self.po = ProductOrder.objects.create(product=self.product, order=self.order, quantity=3, price=1000)

    @mock.patch('django_shop.models.render_to_string')
    def test_order(self, m_render):
        m_render.return_value = 'message'
        self.assertEqual(self.order.receiver, 'Имя: Имя\nEmail: test@test.com\nТелефон: +79991234567')
        ...
        m_render.assert_has_calls([
            mock.call('django_shop/email/order_cash_manager_email.txt', {'order': self.order}),
            mock.call('django_shop/email/order_cash_customer_email.txt', {'order': self.order}),
        ], any_order=True)
    ...
    def test_by_order_id(self):
        self.assertEqual(Order.get_by_order_id(str(self.order.uuid)), self.order)
    ...
    def test_from_form(self, m_render, m_pay):
        ...
        receiver_form = dict(name='Имя', email='test@test.com', phone='+79991234567')
        address_form = {}
        session_key = '12345'
        pm = PaymentType.objects.create(name='Кеш', payment_type=PaymentType.TYPE_ONLINE)
        order_form = {'delivery_type': self.dt, 'payment_type': pm, 'delivery_options': ''}
        products_form = [{'product': self.product, 'amount': Decimal(3)}]
        order = Order.from_forms(products_form, order_form, receiver_form, address_form, session_key)
        ...
        self.assertEqual(po.stock_reduced, True)
        m_render.assert_has_calls([
            mock.call('django_shop/email/order_nc_customer_email.txt', {'order': order}),
            mock.call('django_shop/email/order_nc_manager_email.txt', {'order': order}),
        ], any_order=True)
