...
class BasketModificationError(Exception):
    def __init__(self, message):
        self.message = message


class Basket(object):
    """Объект корзины клиента. Данные о покупках хранятся в сессии на стороне сервера"""
    ...
    def __init__(self, session):
        ...
        if self.BASKET in self._session:
            self._products = self.load(self._session[self.BASKET])
        else:
            self._products = []
    ...
    def save(self):
        ...
        self._session[self.BASKET] = data

    @staticmethod
    def load(basket_data):
        """Загрузить из списка с данными о состоянии корзины структуру, описывающую корзину в классе

        :type basket_data: list[(int, float)]

        :rtype: list[(shop.models.Product, decimal.Decimal)]
        """
        result = []
    ...
    def _check_amount(product_obj, amount):
        if not product_obj.is_weight and not float(amount).is_integer():
            raise BasketModificationError(
                'Этот товар нельзя купить частично'
            )
        if product_obj.is_unlimited:
            return
        if product_obj.in_stock < amount:
            raise BasketModificationError(
                'К сожалению, на складе недостаточно товара для добавления в '
                'корзину'
            )
    ...
    @property
    def measures(self):
        """Измерения прямоуголной коробки, в которую поместятся все продукты в корзине"""
        return 30, 30, 40
