...
class ProductCategoryDetailView(DetailView):
    ...
    def get_queryset(self):
        return self.model.objects.get_available().prefetch_related('products')
...
class ProductDetailView(DetailView):
    ...
    def get_queryset(self):
        self.category = get_object_or_404(ProductCategory,
                                          slug=self.kwargs.get('cat_slug'))
        qs = super(ProductDetailView, self).get_queryset().filter(
            categories__pk=self.category.id)
