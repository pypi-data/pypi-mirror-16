...
class SealightsPlugin(object):
    ...
    def pytest_runtest_setup(self, item):
        try:
            self.test_listener.start_test(item.nodeid)
        except Exception as e:
    ...
    def pytest_report_teststatus(self, report):
        try:
            ...
            if report.when == "teardown":
                ...
                if passed:
                    self.test_listener.passed_test(report.nodeid, report.duration)
                elif skipped:
                    self.test_listener.skipped_test(report.nodeid, report.duration)
                elif failed:
                    self.test_listener.failed_test(report.nodeid, report.duration)
        except Exception as e:
...
def pytest_addoption(parser):
    ...
    parser.addoption("--server", required=True, help="Sealights Server. Must be of the form: http://<server>/api")
