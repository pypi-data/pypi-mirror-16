    ...
    def process_message(self, peer, mailfrom, rcpttos, data):
        ...
        if options.bcc2to in rcpt:
            ...
            if options.alowedre.match(mailfrom):
                ...
                lines = data.split('\n')
                if options.debug >= 3: print("===> Bcc2To rcpt:", repr(rcpt), '' if len(rcpttos) == len(rcpt) else '(truncated)', file=DEBUGSTREAM)
                # Inspect headers
                i = 0
                ...
                tto = to.intersection(rcpt)
                tcc = cc.difference(tto).intersection(rcpt)
                bcc = rcpt.difference(tto, tcc)
                ...
                if toBeg is None:
                    ...
                    if ccBeg is None:
                        ...
                        if toBeg < ccBeg:
                            ...
                            lines[ccBeg:ccEnd] = []
                if options.debug >=2:
                    print("===> To:", tto, file=DEBUGSTREAM)
                    print("===> Cc:", tcc, file=DEBUGSTREAM)
                    print("===> Bcc:", bcc, file=DEBUGSTREAM)
                for a in bcc:
                    hd = mkheader("To", [a])
                    l = len(hd)
                    lines[i:i] = hd
                    data = NEWLINE.join(lines)
                    self._deliver(mailfrom, list([a]), data)
                    lines[i:i+l] = []
                lines[i:i] = mkheader("To", list(tto), fto) + mkheader("Cc", list(cc), fcc)
                data = NEWLINE.join(lines)
                rcpt = tto.union(tcc)
            else:       # to our address, but sender mismatch
                if options.debug >= 1: print("===> DENIED: user '%s'" % mailfrom, file=DEBUGSTREAM)
                log.info("DENIED: user '%s'", mailfrom)
        if len(rcpt):
            refused = self._deliver(mailfrom, list(rcpt), data)
            if len(refused) != 0:
