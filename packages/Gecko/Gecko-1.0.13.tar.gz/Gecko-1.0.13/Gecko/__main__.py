...
def run(args):
    ...
    for sensorname, sensor in sensors.iteritems():
        ...
        scheduler.add_job(sensor.getVal,  args = [s, gecko] ) # Run initial sensor grab right away
        scheduler.add_job(sensor.getVal, 'interval', args = [s, gecko], seconds=sensor.report_rate, max_instances=3, name=s.__class__.__name__)  # set job for future runs
