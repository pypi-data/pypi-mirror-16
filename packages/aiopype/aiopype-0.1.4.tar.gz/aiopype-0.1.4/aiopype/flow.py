...
class Flow(object):
  ...
  def __init__(self, config):
    ...
    self.loop.set_exception_handler(self.default_handler)

  def default_handler(self, _, data):
    """
    Entrypoint for event loop's unhandled exceptions.
    """
  ...
  def handle_exception(self, exception):
    ...
    Not all exceptions are caught if they are not awaited. Since there are some
    background jobs that will run in parallel with the main task, they might have to
