...
class PusherClientSource(Processor):
  """
  Bitstamp Order Book API Processor.

  Processes bitstamp's API and saves data on an OrderBook object,
  while serving as an interface to the live OrderBook object.
  """
  pusher_id = None
  ...
  def __init__(self, name, handler, **kwargs):
    ...
    self.pusher = PusherClient(self.pusher_id)

  async def start(self):
    """
    Initializes the Pusher client, the live order book and the HTTP connection.
    """
