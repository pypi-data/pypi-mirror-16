...
class DJMBotView(generic.View):
    ...
    def _handle_messaging(self, messaging):
        ...
        logger.debug('*****************************************')
        logger.debug('%s' % messaging)
        handlers = policy.get_handlers(messaging.get_receiving_type())
        logger.debug(
            'Ready to process messaging of ReceivingType %s' %
            messaging.get_receiving_type())
        ...
        for handler in handlers:
            if not issubclass(handler.get_class(), BaseHandler):
                continue
            logger.debug('Routing message %s to handler %s' %
                         (messaging, handler))
            ...
            if handler_instance.should_handle():
                ...
                senders = policy.get_senders(messaging.get_receiving_type(),
                                             handler)
                logger.debug('Handler jobs are done, ready to send using senders: '
                             '%s' % senders)
                for sender in senders:
                    if not issubclass(sender.get_class(), CommonSender):
                        continue
                    logger.debug('Sending to user using sender %s' % sender)
                    sender_instance = sender.get_class()(messaging.get_psid(),
                                                         **sender.get_args())
