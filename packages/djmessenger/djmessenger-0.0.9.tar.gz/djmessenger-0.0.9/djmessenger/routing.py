...
class TargetClass(Serializable):
    ...
    def __repr__(self):
        return 'class %s with args %s' % (self.name, self.args)
    ...
    def get_handlers(self, rtype):
        ...
        if self.get_router(ReceivingType.DEFAULT):
            for handler in self.get_router(ReceivingType.DEFAULT).handlers:
                handlers_dict[handler.get_class()] = handler
        if self.get_router(rtype) is not None:
            for handler in self.get_router(rtype).handlers:
                handlers_dict[handler.get_class()] = handler
        if UserProfileHandler in handlers_dict:
            ret.append(handlers_dict[UserProfileHandler])
            del handlers_dict[UserProfileHandler]
        for _, handler in handlers_dict.items():
            ret.append(handler)
        return ret
