...
def listMultipleSelection(title, prompt, list):
    """ Applescript generated list that allows multiple selections. Returns array of results."""
    ...
    template = template.strip().replace(' ', '').split(',')

    return macman.misc.osascript(template)
