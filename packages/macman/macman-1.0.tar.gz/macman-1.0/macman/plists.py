...
def checkPlist(plists):
    ...
    for scripted_plist_path, scripted_plist in plists.iteritems():
        to_write = False
        services = None

        # replace tilde with user home path
        if scripted_plist_path.startswith('~'):
            scripted_plist_path = macman.misc.tildeToPath(macman.users.getCurrentUser(), scripted_plist_path)

        # read current plist contents into dictionary
        current_plist = readPlist(scripted_plist_path)

        for key, value in scripted_plist.iteritems():
            ...
            # if value is not a dictionary
                ...
                try:
                    if current_plist[key] != value:
                        ...
                        macman.logs.writeLog('%s "%s" incorrect, setting value to: "%s"' % (scripted_plist_path, key, str(value)))

                # if scripted key doesn't exist in current plist
                except (KeyError, TypeError):
                    to_write = True
                    current_plist[key] = value
                    macman.logs.writeLog('%s "%s" does not exist, setting value to: "%s"' % (scripted_plist_path, key, str(value)))
                ...
                try:
                    ...
                    for dict_key, dict_value in value.iteritems():
                        if client_id == dict_key and current_plist[key] != value[client_id]:
                            ...
                            macman.logs.writeLog('%s "%s" incorrect, setting value to: "%s"' % (scripted_plist_path, key,
                                                                                                str(value[client_id])))
                            break
                ...
                # if key doesn't exist
                    ...
                    for dict_key, dict_value in value.iteritems():
                        if client_id == dict_key:
                            ...
                            macman.logs.writeLog('%s "%s" does not exist, setting value to: "%s"' % (scripted_plist_path, key,
                                                                                                     str(dict_value)))

        # Write dictionary to plist
        if to_write:
            writePlist(scripted_plist_path, current_plist, services)
    ...
    if os.path.isfile(plist_path):
        ...
        if not os.access(plist_path, os.R_OK):
            ...
            # copy to temp file
                ...
                plist_path = macman.files.createTemporaryCopy(plist_path)
        ...
        if not key:
            ...
            # when plist format is binary it must be converted to xml before reading
                ...
                # convert to xml
                current_plist = macman.misc.plutil('-convert', 'xml1', '-o', '-', plist_path)
        ...
        # if key provided
            ...
            # when plist format is binary it must be converted to xml before reading
                ...
                # convert to xml
                current_plist = macman.misc.plutil('-convert', 'xml1', '-o', '-', plist_path)[key]
...
def writePlist(plist_path, plist_dict, services=None):
    ...
    # retrieve current plist file permissions
    plist_permissions = macman.files.getPathPermissions(plist_path)

    # if no permissions returned (file doesn't exist) set owner to current user
    uid = plist_permissions['uid'] if not plist_permissions['uid'] else macman.users.getUserID(macman.users.getCurrentUser())
    gid = plist_permissions['gid'] if not plist_permissions['gid'] else 20
    mode = plist_permissions['mode'] if not plist_permissions['mode'] else 0774
...
def binaryToXml(plist_path):
    """ Convert a binary formatted plist to xml format """

    return macman.misc.plutil('-convert', 'xml1', '%s' % plist_path)


def xmlToBinary(plist_path):
    """ convert a xml formatted plist to binary """
