...
def getUserPasswordHash(username):
    ...
    output = macman.misc.defaults('read', '/var/db/dslocal/nodes/Default/users/%s.plist' % username, 'ShadowHashData')
...
def setUserPasswordHash(username, password_hash):
    ...
    macman.misc.defaults('write', '/var/db/dslocal/nodes/Default/users/%s.plist' % username, 'ShadowHashData', password_hash)
