...
class AmqpClient(object):
        ...
        def open_channel():
            try:
                ...
                if type(e) is not MissingProtocol:
                    self.logger.exception("open_channel %s error: %s", channel_id, e.message)
    ...
    def setup(self):
        ...
        try:
            ...
            if type(e) is not MissingProtocol:
                self.logger.exception("error while setting up channel id %s: %s", self.channel_id, e.message)
    ...
    def teardown(self):
        ...
        if self.factory.p:
            try:
                ch = yield self.factory.p.channel(self.channel_id)
                yield ch.channel_close()
                ch.close("facade teardown")
                yield self.factory.p.remove_channel(self.channel_id)
            except Exception as e:
    ...
    def basic_publish(self, msg, exchange=None, routing_key=None, properties=None, confirm=False):
        ...
        routing_key = routing_key or get_routing_key(exchange_name, self.bindings)
        if confirm:
