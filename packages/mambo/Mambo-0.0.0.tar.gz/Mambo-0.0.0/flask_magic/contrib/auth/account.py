...
from flask_magic import (Magic, models, set_nav, page_meta, mail, init_app,
                         ...
                         abort, recaptcha, storage, models)
...
from . import (current_user, authenticated, logout_user,
               ...
               email_reset_password, email_verification_email,
...
def setup(**kwargs):
    ...
    @set_nav(title=nav_kwargs.pop("title", "My Account") or "My Account",
             ...
             **nav_kwargs)
    class AuthAccount(Magic):
        base_route = kwargs.get("route") or "/"
        decorators = [authenticated] + kwargs.get("decorators")
        ...
        @set_nav("Logout", endpoint="AuthLogin:logout", order=100)
        def _(self):
            pass

        @set_nav("Account Info", order=1)
        def account_info(self):
            page_meta(title="Account Info")

        @set_nav("Edit Account Info", visible=False)
        @get()
        @post()
        ...
        @set_nav("Change Password", order=3)
        @get()
        @post()
        ...
        @set_nav("Change Profile Image", order=4)
        @get()
        @post()
        ...
        @set_nav("Setup Login", visible=False)
        @get()
        @post()
