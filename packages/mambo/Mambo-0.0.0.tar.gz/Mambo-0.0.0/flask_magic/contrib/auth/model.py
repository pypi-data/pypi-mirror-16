...
import datetime
from flask_magic import Magic, register_models, utils
from flask_magic.exceptions import ModelError
...
def setup(**kwargs):
    ...
    class AuthRole(db.Model):
        ...
        def _setup_model(cls):
            """
            Flask-Magic specific
            To setup some models data after
            :return:
            """
            [cls.new(level=r[0], name=r[1]) for r in cls.DEFAULT]
    ...
    class AuthUserLogin(db.Model):
        ...
        def new(cls, login_type, user_id=None, email=None, password=None,
            ...
            if not user_id:
                role = user_info.get("role", AuthRole.MEMBER)
        ...
        def change_password(self, password=None, random=False):
            """
    ...
    register_models(AuthUser=AuthUser,
                    AuthRole=AuthRole,
