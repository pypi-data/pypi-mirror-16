...
class UsergridResponse(object):
    def __init__(self, api_response, client):
        ...
        if api_response is None:
            ...
            self.ok = api_response.status_code == 200
            ...
            self.entity_array = self.body.get('entities', [])
            self.error = self.body.get('error', 'not available')
            self.error_description = self.body.get('error_description', 'not available')
            self.exception = self.body.get('exception', 'not available')
    ...
    def is_duplicate_unique_value_error(self):
        return self.error in ['duplicate_unique_property_exists']
