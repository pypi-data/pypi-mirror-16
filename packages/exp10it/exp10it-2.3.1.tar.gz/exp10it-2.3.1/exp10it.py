...
def check_webshell_uri(uri):
    ...
    #第二个键为webshell爆破方式,用y2表示,y2的值可能是"biaodan_bao"(根据搜到的表单爆)或者"direct_bao"(直接爆)或者""(空字符串,对应uri不是webshell)
    ...
    strange_filename_pattern=re.compile(r"^(http).*(((\d){3,})|(/c99)|(/x)|(/css)|(/licen{0,1}se(1|2){0,1}s{0,1})|(hack)|(fuck)|(h4ck)|(/diy)|(/wei)|(/2006)|(/newasp)|(/myup)|(/log)|(/phpspy)|(/b374k)|(/80sec)|(/90sec)|(/r57)|(/b4che10r)|(X14ob-Sh3ll)|(aspxspy)|(server_sync))\.((php(3|4|5){0,1})|(phtml)|(asp)|(asa)|(cer)|(cdx)|(aspx)|(ashx)|(asmx)|(ascx)|(jsp)|(jspx)|(jspf))$",re.I)
    if re.match(strange_filename_pattern,uri) and belong2github==False and len(content)<8000:
        print "strang filename"
        y1=True
    ...
    if y1==True:
        if code==str(200) and len(content)<8000 and re.search(r'''method=('|")post('|")''',content):
            y2="biaodan_bao"
        if code==str(200) and len(content)==0:
            y2="direct_bao"
