...
def check_webshell_uri(uri):
    ...
    #y3的值是一个字典{"code":"","title":"","content":""}   其中code的类型为str
    ...
    return {'y1':y1,'y2':'%s' % y2,'y3':{"code":"%s" % code,"title":"%s" % title,"content":"%s" % content}}
...
def get_form_from_html(html):
    #从html内容中获取所有的form表单
    #返回结果为一个字典,包含2个键值对
    #"user_form_name":"" 如果没有则为空
    #"pass_form_name":"" 如果没有则为空
...
def get_form_from_uri(uri):
    ...
    content=get_response_key_value_from_uri(uri)['content']
    return get_form_from_html(content)



def crack_php_direct_webshell_uri(uri,pass_dict_file):
    #爆破php的一句话类型的webshell
    ...
    def php_direct_webshell_crack_thread((password,uri)):
        global get_flag
        global try_time
        global start
        if get_flag==1:
        ...
        #print "%s/%s times password: %s cracking webshell --->%s" % (str(i),sum,password,uri)
        values = {'%s' % password : 'echo 29289;'}
        #values = {'%s' % password : 'phpinfo();'}
        data = urllib.urlencode(values)

        try_time+=1
        #print "password: %s cracking webshell --->%s" % (password,uri)
        ...
        print "%s/%s password: (%s)  finished attempt crack webshll--->(%s)" % (str(try_time),sum,password,uri)
    ...
    def crack_php_direct_webshell_uri_inside_func(uri,pass_dict_file):
        uris=[]
        passwords=[]
        global sum
        global start
        ...
        pool=ThreadPool(20)
        results_list=pool.map_async(php_direct_webshell_crack_thread,zip(passwords,uris),2000)
        pool.close()
        pool.join()

    crack_php_direct_webshell_uri_inside_func(uri,pass_dict_file)


def crack_php_biaodan_webshell_uri():
    pass
def crack_asp_direct_webshell_uri():
    pass
def crack_asp_biaodan_webshell_uri():
    pass
def crack_aspx_direct_webshell_uri():
    pass
def crack_aspx_biaodan_webshell_uri():
    pass
def crack_jsp_direct_webshell_uri():
    pass
def crack_jsp_biaodan_webshell_uri():
    pass


def crack_admin_uri():
    pass
def crack_biaodan_webshell_uri(uri,user_dict,pass_dict):
    user_dict="dicts/user.txt"
    pass_dict="dicts/webshell_passwords.txt"
    username_list=[]
    password_list=[]
    ...
    if user_form_name!="":
        while 1:
            if os.path.exists(user_dict) is False:
                print "please input your username dict:>",
                user_dict=raw_input()
                if os.path.exists(user_dict) is True:
                    break
            else:
                break
