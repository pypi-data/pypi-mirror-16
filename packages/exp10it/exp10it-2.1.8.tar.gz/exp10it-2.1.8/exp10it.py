...
def get_response_key_value_from_uri(uri):
    ...
    br.open(uri)
    response_code=br.response().code
    response_title=br.title()
    response_content=br.response().read()
    return {'code':'%s' % response_code,'title':'%s' % response_title,'content':'%s' % response_content}
