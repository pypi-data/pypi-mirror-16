...
class Connection(object):
    ...
    def update_addressbook(self, message):
        u"""
        ...
        :return: None
        """
        database_file = os.path.join(LocalConfigChecker.contacts_directory, self.identifier, u"addressbook.db")
        database = sqlite3.connect(database_file)

        query = u"SELECT * FROM Contacts WHERE ? = address"
        query_result = database.execute(query, (message.address,)).fetchall()
        if len(query_result) == 0:

            PrintLogger.print(u"Adding Contact to Database: " + message.name, 3)
            insertion = u"INSERT INTO Contacts (address, name) VALUES(?, ?)"
