...
def get_stock_hist_bar(code, freq="d", start_date=None, end_date=None, adjusted="f", field=None):    """获取股票历史时间段K线数据    Parameters    ----------    code:str
    ...
    if not isinstance(code, tuple([str , list])):
        raise TypeError("get_stock_csf_industry() arg code must be a str  or  list types")
    params["codes"] = __format_param("code", code)
    ...
    if not isinstance(code, tuple([str , list])):
        raise TypeError("get_stock_industry() arg code must be a str  or  list types")
    params["codes"] = __format_param("code", code)
    ...
    type:str 类型    Examples    --------    >>> import csf
    >>> csf.config.set_token(csf.config.EXAMPLES_ACCESS_KEY,csf.config.EXAMPLES_SECRET_KEY)
    >>> csf.get_macro_data(marco_code="A02090N0503",start_date="2005-01-01",end_date="2016-06-01")    """    params = dict()
