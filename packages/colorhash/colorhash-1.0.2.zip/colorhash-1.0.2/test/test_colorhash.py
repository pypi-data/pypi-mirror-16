...
class TestCrc32Hash(unittest.TestCase):
    def test_eq(self):
        x1 = 'Hello World'
        x2 = 'Hello World'
        self.assertEqual(colorhash.crc32_hash(x1), colorhash.crc32_hash(x2))

    def test_across_python_instance(self):
        p = subprocess.Popen(
            [
                sys.executable, '-c',
                'import colorhash; print(colorhash.crc32_hash("Hello World"))'
            ],
            stdout=subprocess.PIPE)
        stdout, _ = p.communicate()
        self.assertEqual(colorhash.crc32_hash('Hello World'), int(stdout))
