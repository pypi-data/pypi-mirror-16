...
class UiSearch:
    ...
    pxResult = Px('''
     <div var="ajaxHookId='queryResult';
               ...
               searchName=field.getName(req)|req.get('search', '');
    ...
    def getName(self, request):
        '''Sometimes, the name of a Search is dynamically known and the one
           stored in self.name (as "_field_" must be ignored).'''
        name = self.name
        if name == '_field_': return request.get('search', name)
        return name
    ...
    def getAjaxData(self, hook, ztool, **params):
        ...
        searchName = self.getName(ztool.REQUEST) # self.name can be "_field_"
