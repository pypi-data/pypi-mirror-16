...
def clean_all(dry_run=False):
    ...
    clean_dangling_volumes(dry_run=dry_run)
...
def clean_dangling_volumes(dry_run=False):
    volumesNotInUse = common.shell_call('docker volume ls -qf dangling=true')

    if volumesNotInUse:
        volumes = volumesNotInUse.replace('\n', ' ')
        if dry_run:
            print('will remove volumesNotInUse {}'.format(volumes))
        else:
            print('removing volumes not in use {}'.format(volumes))
            common.shell_call('docker volume rm $(docker volume ls -qf dangling=true)')
    else:
        print('no volumesNotInUse to remove')
