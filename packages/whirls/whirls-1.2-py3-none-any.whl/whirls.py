    ...
    def _handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == MOUSE_LEFT:
            if self._gui_enabled:
                x, y = event.pos
                if x > self._slider_gui.rect.w or y > self._slider_gui.rect.h:
                    self._container.remove(self._slider_gui)
                    self._gui_enabled = False
                    return True
            else:
                self._container.add(self._slider_gui, x=0, y=0)
...
class Painter:
    ...
    def __init__(self, screen_size: Tuple[int, int], slider_gui: 'SliderGui', inner_point_count: int, inner_border: int,
        ...
        self._draw_speed = 0.03
        self._draw_acceleration = 1.01
    ...
    def _move(self) -> None:
        ...
        if self.partial_draw:
            self._max_lines = self._max_lines * self._draw_acceleration + self._draw_speed
...
class FloatHSlider:
    ...
    def __init__(self, value: float, float_min: float, float_max: float, size: float, **kwargs) -> None:
        ...
        self._int_min = 0
        self._int_max = int(kwargs.get('width', 1000))
        int_value = interp(value, float_min, float_max, self._int_min, self._int_max)
        self.slider = gui.HSlider(int_value, self._int_min, self._int_max, size, **kwargs)  # type: gui.HSlider
    ...
    def value(self) -> float:
        ...
        return interp(self.slider.value, self._int_min, self._int_max, self._float_min, self._float_max)
