...
* There are two options available for inputting data to Input Raster.  The first option is to select data from a directory. For this option to succeed, the data must be accessible by the GSF Server as well.  The second option is to input an http data source that GSF can handle.  By default GSF has some data available at an http://localhost:9191/ese/data location. To test the tool generation, choose the http option.

.. image:: images/spectral_index_input.png

* Click OK, and when the tool finishes processing, a result appears.  Debug Tip: To see if GSF server received the job and to check the job status, go to http://localhost:9191/job-console/
...
* Click run and the tool finishes generating when new toolbox, navigate in the Project tab to the location specified in Output Toolbox. 
* Double-click on Spectral Index and the tool appears with two required input parameters and one optional parameter. 

.. image:: images/spectral_index_arcgispro.png

* There is two options available for inputting data to Input Raster.  The first option is to select data from a directory. For this option to succeed, the data must be accessible by the GSF Server as well.  The second option is to input an http data source that GSF can handle.  By default GSF has some data available at an http://localhost:9191/ese/data location. To test the tool generation, choose the http option.
...
selecting the GSF Py 64-bit option. After install of GSF Py on the ArcGIS Server, 
restart the ArcGIS Server service to make GSF Py client Python libraries available
to GPServices. 
Note: The ArcGIS Server publishing tools are only available in ArcMap.
 
If ArcGIS Server is not co-located with the GSF Server, enter the GSF server name instead of localhost when creating the tool. See instructions above to generate and run a tool.
