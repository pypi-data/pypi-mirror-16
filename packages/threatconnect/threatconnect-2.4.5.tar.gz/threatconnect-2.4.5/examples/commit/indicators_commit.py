...
""" custom """
from threatconnect import ThreatConnect, ObservationObject
...
def main():
    ...
    # resource.add_attribute('Description', 'TEST attribute #{0:d}'.format(rn))
    #
    # # (Optional) add tag to newly createdd resource
    # resource.add_tag('TAG #{0:d}'.format(rn))
    # resource.add_tag('EXAMPLE')
    #
    # # (Optional) set security label to newly created resource
    ...
    # (Optional) add observation to resource
    resource.add_observation(1, '2016-05-04T13:06:00Z')
    ...
    resource.load_observation_count()

    print "Observation Count: {}".format(resource.observation_count)
    print "Getting observations"
    for obs in resource.observations:
        print "Count: {}\nDate: {}\n\n".format(obs.count, obs.date_observed)

    #
    # add false positive and observation (to a host)
    #
    # this requires that the resource was instantiated at the beginning of the script.
    # resource = resources.add('{0!s}_{1!s}'.format(prefixes['host'], str(rn).zfill(3)), owner)
    #
    # resource.set_confidence(rn)
    # resource.set_rating(randint(1, 5))
    #
