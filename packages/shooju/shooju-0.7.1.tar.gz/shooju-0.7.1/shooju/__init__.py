...
class Connection(object):
    ...
    def _post_bulk(self, bulk_data, job_id=None, collision_retry_timeout=60):
        """
        ...
        """
        return self.shooju_api.bulk_series(bulk_data, job_id, collision_retry_timeout)
    ...
    def _process_search_request(self, response, key_field=None):
        ...
        if has_points and has_fields:
            ...
            data = {
                ...
                'date':  list(map(unixtime_to_datetime, series_dates * num_of_series)),
...
class ShoojuApi(object):
    ...
    def get_series(self, series_id, date_start=None, date_finish=None,
        ...
        params = {
            ...
            'snapshot_date': to_unixtime(snapshot_date) if snapshot_date else None,
            'reported_date': to_unixtime(reported_date) if reported_date else None,
        }
    ...
    def bulk_series(self, requests, job_id=None, collision_retry_timeout=60):
        """
        ...
        while req_index_to_send and attempts_left:
            ...
            for i, resp in enumerate(self.client.post(
                    self.API_SERIES_BULK, ignore_exceptions=True, data_json=payload, params=params)['responses']):
...
class Client(object):
    ...
    def _call(self, path=None, ignore_exceptions=False, **kwargs):
        """
        ...
        try:
            r = method(url, params=kwargs.get('params'), data=payload, headers=headers,
            ...
            if attempt >= self._retries:
                ...
                return self._call(path, ignore_exceptions=ignore_exceptions, files=files, **kwargs)
...
class BaseJob(object):
    def __init__(self, conn, job_id, batch_size, pre_hooks=None, post_hooks=None, collision_retry_timeout=60):
        """
...
class RemoteJob(BaseJob):
    ...
    def submit(self):
        ...
        if bulk_data:
            try:
                response = self._conn._post_bulk(bulk_data, self._job_id, self._collision_retry_timeout)
            finally:
                # always flush cache
                self._values = {}
                self._cur_batch_size = 0
