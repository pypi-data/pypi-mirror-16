...
from util import get_description_from_tupple
import exceptions
...
def bind_address_on_database(
    self, database_bind, user=None, retries=4, exponential_factor=2,
    time_facor_in_seconds=60
):
    ...
    task_history.update_details(
        persist=True, details="Creating binds for {} on {}.".format(
    ...
    try:
        ...
        for attempt in (i**exponential_factor for i in range(1, retries)):
            ...
            if operation_completed:
                ...
                sleep(attempt * time_facor_in_seconds)
    ...
    try:
        ...
        for job in AclApiJob.objects.filter(
            ...
            if 'jobs' in response:
                ...
                job_status = get_description_from_tupple(JOB_STATUS, status)
