...
class CWinSVProgram:
    ...
    def CheckAndRunOneCmd(self):
        ...
        if self.CheckStopFileExists():
            ...
            #self.Process = psutil.Popen(sCmd, stdout=self.fdOut,stderr=self.fdErr, shell=False)
            self.Process = psutil.Popen(sCmd, stdout=self.fdOut,stderr=self.fdErr, shell=True)
