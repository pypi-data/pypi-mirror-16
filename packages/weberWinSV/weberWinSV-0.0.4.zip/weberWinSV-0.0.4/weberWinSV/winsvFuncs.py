...
def GetAbsPathFmLocal(sLocalPath=".", sFName=""):
    import os, inspect
    sAbsPath = os.path.realpath(os.path.abspath(os.path.join(
        os.path.split(inspect.getfile( inspect.currentframe()))[0],sLocalPath)))
    sAbsPath += os.sep
    if sFName:
...
#--------------------------------------
def testMain():
    print PrettyPrintStr(LoadWinSVConfigFmFile('demoSettingsWinSV.py','gDictConfigByGroupId',
