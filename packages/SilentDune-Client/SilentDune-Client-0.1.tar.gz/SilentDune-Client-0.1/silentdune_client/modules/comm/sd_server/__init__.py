...
class SilentDuneServerModule(BaseModule):
    ...
    def add_installer_arguments(self, parser):
        ...
        group.add_argument('--server-mod-enable', action='store_false', help=_('Enable the server module'))  # noqa
    ...
    def validate_arguments(self, args):
        ...
        if (not '--server-mod-enable' in sys.argv) and (
            ...
            print('sdc-install: argument --server-mod-enable conflicts with other server module arguments.')
            return False
    ...
    def service_connect_to_server(self):
        ...
        self._sds_conn = SilentDuneConnection(server, no_tls, port)
    ...
    def service_startup(self):
        ...
        if self._sds_conn.status == ConnStatus.not_registered:
            _logger.error('{0} this node is not registered, unable to start.'.format(self.get_name()))
            return False

        # Set our server check interval value
