...
# All rights reserved.
# <LICENSE=ISC>
"""BaseController API Version 0.6.0
...
import sys,os,urllib,logging,datetime
...
class BaseController(BaseControllerMixIn):
    ...
    def init_request(self, environ={}, request_class=HTTPRequest):
        ...
        try:
            ...
            # Invalid environment, create one from scratch
            self.logger.debug('recreating environment...')
            _locals.environ = environ

        _locals.request = request_class(self.environ.copy())
        #if self.debug:
    ...
    def get_request(self):
        return getattr(_locals, 'request')
    ...
    def _environ_getter(self):
        ...
        return getattr(_locals, 'environ')
