...
def parse_cmdline(argv):
    ...
    parser.add_argument('--disable-auth', action="store_true", \
        dest="disable_auth", \
        help="Disable auth middleware backend", default=False)
...
def main(argv):
    ...
    if options.disable_auth is not True:
        ...
        from notmm.controllers.auth import SimpleAuthController
        _('legacy auth enabled')
        #auth_conf = global_conf[auth_module]
        #auth_conf['handle_exceptions'] = False
        wsgi_app = SimpleAuthController(wsgi_app, auth_conf=auth_conf, settings=settings)
    else:
