...
class Clean(ICommand):
    ...
    def build(self, subparser):
        ...
        logging.debug("- [clean] command has been added to argparse")

        #   add clean all command
        super().build(subparser)
        cmd = subparser.add_parser("cleanall", help="Help")
        cmd.set_defaults(func=self.run)
        logging.debug("- [cleanall] command has been added to argparse")
