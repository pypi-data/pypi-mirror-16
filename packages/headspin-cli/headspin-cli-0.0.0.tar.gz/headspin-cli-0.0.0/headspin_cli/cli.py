...
Usage:
  ...
  hs session start http_proxy <device_id> [<companion_uuid>] [--json]
  hs session start network_container <device_id> [--json]
  hs session start device_container <device_id> [--json]
  hs session stop <session_uuid> [--json]
  hs session rm <session_uuid> [--json]
...
Detailed Description:
  ...
  hs session start http_proxy <device_id> [<companion_uuid>]

        Starts a HeadSpin HTTP proxy session on a device specified by
        <device_id>, if such a device is available. If a
        <companion_uuid> is specified, the session is started in
        capture mode the companion is connected to the session,
        provided that the companion is currently connected to the
        HeadSpin platform but not connected to any session. Use the
        `companion ls` command to view the currently connected
        companions.

  hs session start network_container <device_id>

        Starts a HeadSpin network container session on a device
        specified by <device_id>. The container can be SSHd into
        and the device's mobile network can be accessed on interface
        eth1. The container's eth0 interface is connected to the
        internet via broadband.

  hs session start device_container <device_id>

        Starts a HeadSpin device container on a device specified by
        <device_id>. In addition to mobile network access on
        eth1, the container has direct access to the device via `adb`.
  ...
  hs companion ls
        ...
        Lists all companions that are currently connected to the
        HeadSpin platform.

  hs companion inspect <companion_uuid>

        Displays details about a given companion.
...
def device_ls(as_json=False):
    ...
    fmt_string = ("{device_id:<30}  {location:<25}  {connection:10}  "
                  + "{carrier:<15}")
    ...
    for device in sorted(body['devices'],
                         key=lambda d: d['connection'] + d['carrier']):
        print(fmt_string.format(
