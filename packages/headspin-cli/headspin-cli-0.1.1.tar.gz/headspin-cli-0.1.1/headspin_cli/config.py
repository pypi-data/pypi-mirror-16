...
class AuthLease(object):
    def __init__(self, user_id, org_title, api_key, secret_api_key):
...
class AuthConfig(object):
    ...
    def has_lease(self, user_id, api_key):
        for lease in self.leases:
            if lease.user_id == user_id and lease.api_key == api_key:
                return True
        return False
...
def add_lease(response_body):
    user_id = response_body['user_id']
    org_title = response_body['org_title']
    api_key = response_body['api_key']
    secret_api_key = response_body['secret_api_key']

    try:
        auth_config = AuthConfig.load_from_file()
        if auth_config.has_lease(user_id, api_key):
        ...
        leases = [AuthLease(user_id, org_title, api_key, secret_api_key)]
