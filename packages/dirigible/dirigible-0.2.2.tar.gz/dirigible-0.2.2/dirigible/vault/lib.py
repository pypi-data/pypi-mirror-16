...
from dirigible.vault.unicode import to_unicode, to_bytes
...
def check_prereqs():
    ...
    if not HAS_AES or not HAS_COUNTER or not HAS_ANY_PBKDF2HMAC or not HAS_HASH:
        raise VaultError(CRYPTO_UPGRADE)
...
class VaultLib:
    ...
    def encrypt(self, data):
        ...
        if self.is_encrypted(b_data):
            raise VaultError("input is already encrypted")
        ...
        try:
            ...
            raise VaultError(u"{0} cipher could not be found".format(self.cipher_name))
        this_cipher = Cipher()
    ...
    def decrypt(self, data):
        ...
        if self.b_password is None:
            raise VaultError("A vault password must be specified to decrypt data")

        if not self.is_encrypted(b_data):
            raise VaultError("input is not encrypted")
        ...
        if cipher_class_name in globals() and self.cipher_name in CIPHER_WHITELIST:
            ...
            raise VaultError("{0} cipher could not be found".format(self.cipher_name))
    ...
    def _format_output(self, b_data):
        ...
        if not self.cipher_name:
            raise VaultError("the cipher must be set before adding a header")
...
class VaultEditor:
    ...
    def decrypt_file(self, filename, output_file=None):
        ...
        try:
            ...
            raise VaultError("%s for %s" % (to_bytes(e),to_bytes(filename)))
    ...
    def create_file(self, filename):
        ...
        if os.path.isfile(filename):
            raise VaultError("%s exists, please use 'edit' instead" % filename)
...
class VaultFile(object):
    ...
    def __init__(self, password, filename):
        ...
        if not os.path.isfile(self.filename):
            raise VaultError("%s does not exist" % self.filename)
        try:
            self.filehandle = open(filename, "rb")
        except Exception as e:
            raise VaultError("Could not open %s: %s" % (self.filename, str(e)))
    ...
    def get_decrypted(self):
        ...
        if self.is_encrypted():
            ...
            if dec_data is None:
                raise VaultError("Failed to decrypt: %s" % self.filename)
            else:
...
class VaultAES:
    ...
    def encrypt(self, data, password, key_length=32):
        ...
        raise VaultError("Encryption disabled for deprecated VaultAES class")
