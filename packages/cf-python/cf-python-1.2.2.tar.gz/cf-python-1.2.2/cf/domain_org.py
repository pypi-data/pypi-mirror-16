...
True
        ...
        '''
        self._axes_sizes = {} # Domain axes
        self.d = {}           # Dimension coordinates
        self.a = {}           # Auxiliary coordinates
        self.m = {}           # Cell measures
        self.r = {}           # Coordinate references
        self.v = {}           # Ancillary variables
        self.c = {}           # Coordinate conversion terms
        self._map = {}
        self._axes = {}
...
'''
    ...
    def _conform_ref(self, rkey, ref):
        '''

Replace the content of ref.coords with domain coordinate identifiers,
where possible.
...
>>> d._conform_ref(r)
        ...
        axes = []
        role = ('d', 'a')        
        for identifier in ref.coordinates.union(ref.coordinate_terms.values()):
            key = self.item(identifier, role=role, exact=True, key=True)
            if key is not None:
                identity_map[identifier] = key
                axes.extend(self.axes(key))
        #--- End: for

        role = ('c',)
        for identifier in ref.ancillary_terms.values():
            key = self.item(identifier, role=role, exact=True, key=True)
            if key is not None:
                identity_map[identifier] = key
                axes.extend(self.axes(key))
        ...
        ref.change_identities(identity_map, i=True)

        self._axes[rkey] = tuple(axes)
    #--- End: def
 
    def _replace_identifier_in_refs(self, key):
        '''
...
>>> d._replace_identifier_in_refs('dim1')
>>> d._replace_identifier_in_refs('aux0')
>>> d._replace_identifier_in_refs('var0')

'''
        identity_map = {key: self.get(key).identity(None)}
...
'''
        if qq == 'axis':
            keys = self._axes_sizes
            item_type = 'dim'
        else:
            keys = getattr(self, item_type[0])
...
'''
        return self.new_identifier('axis')
    #--- End: def
...
'''
        return self.new_identifier('aux')
    #--- End: def

    def new_ancillary_identifier(self):
        '''

'''
        return self.new_identifier('var')
    #--- End: def

    def new_coordinate_conversion_variable_identifier(self):
        '''

'''
        return self.new_identifier('ccv')
    #--- End: def
...
'''
        return self.new_identifier('msr')
    #--- End: def
...
'''
        return self.new_identifier('ref')
    #--- End: def
...
(variable.__class__.__name__, variable_shape))
    ...
    def _asd(self, variable, role, axes, scalar=False):
        '''
...
'''
        if strict_axes:
            axes_all = axes
            print "WARNING: strict_axes has been deprecated. Replacing the axes parameter with the axes_all parameter."

        pool = {}
        for r in ('d', 'a', 'm', 'r', 'v', 'c'):
...
'''
    ...
    def _insert_ancillary(self, item, key=None, axes=None, copy=True, replace=True):
        '''
...
'''
        if not key:
            key = self.new_identifier('var')

        if axes is None:
            raise ValueError("Must set axes for ancillary variable")

        item = self._insert_item(item, key, 'auxiliary variable', axes=axes,
                                 copy=copy, scalar=True)
...
>>>
    ...
    def insert_ancillary(self, item, key=None, axes=None, copy=True, replace=True):
        '''Insert a XXXXXXXXXXxx into the domain in place.
...
>>>
        ...
        '''
        if key is None:
            key = self.new_ancillary_identifier()

        item = self._insert_item(item, key, 'ancillary variable', axes=axes,
                                 copy=copy)
    ...
    def insert_ref_ancillary(self, item, key=None, axes=None, copy=True, replace=True):
        '''Insert a XXXXXXXXXXxx into the domain in place.
...
>>>
        ...
        '''
        if key is None:
            key = self.new_coordinate_conversion_variable_identifier()

        item = self._insert_item(item, key, 'coordinate conversion variable', axes=axes,
                                 copy=copy)
    ...
    def insert_ref(self, item, key=None, axes=None, copy=True, replace=False):
        '''
...
'''
        if key is None:
            # Create a new domain coordinate reference identifier
            key = self.new_ref_identifier()
        elif not replace and key in self.r:
            raise ValueError(
"Can't insert coordinate reference object: replace={0} and key {1!r} already exists".format(
    replace, key))
        
        if copy:
            item = item.copy()
  
        self._conform_ref(key, item)
...
See `cf.Field.remove_items`.
        ...
        for key, item in self.items(items, **kwargs).iteritems():
            ...
            # If the removed item is a dimension coordinate, auxiliary
            # coordinate or ancillary variable then ...
            if x in 'dac':
                self._replace_identifier_in_refs(key)
