...
class DvrMache(object):
  ...
  def put(self, data, key):
    Id = str(uuid.uuid4())
    array_id = self._change_record("$set", data, key, Id)
    return Id
  ...
  def delete(self, reference_id):
    db = self.connect()
    collection = db["data"]
    dvr_logger.lager.info("Deleting contents in data collection.")
    if reference_id == "all":
      reference_id = {}
      response = collection.delete_many(reference_id)
    else:
      response = collection.delete_many({"_id": reference_id})
