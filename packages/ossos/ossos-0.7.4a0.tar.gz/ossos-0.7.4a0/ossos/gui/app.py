...
def create_application(task_name, working_directory, output_directory,
    ...
    if task_name == tasks.CANDS_TASK:
        ...
        ProcessRealsApplication(working_directory, output_directory,
                                dry_run=dry_run, debug=debug, name_filter=name_filter, user_id=user_id, zoom=zoom)
    elif task_name  == tasks.STATIONARY_TASK:
        ProcessStationaryApplication(working_directory, output_directory, dry_run=dry_run,
...
class ProcessStationaryApplication(ProcessCandidatesApplication):

    @property
    def input_suffix(self):
        return tasks.suffixes[tasks.STATIONARY_TASK]
    ...
    def _create_workunit_builder(self,
                                 ...
                                 output_context,
                                 progress_manager):
        return CandidatesWorkUnitBuilder(StationaryParser(), input_context, output_context, progress_manager, dry_run=self.dry_run)
