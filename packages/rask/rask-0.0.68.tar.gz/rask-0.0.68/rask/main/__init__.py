...
class Main(Base):
    ...
    def http(self):
        try:
            ...
            self.__http_app__ = Application(self.__http_routes__,autoreload=options.autoreload)
            self.__http__ = self.__http_app__.listen(options.rask['http']['port'])
            self.__http__.xheaders = True
