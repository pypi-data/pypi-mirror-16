...
def setup_temp_dir():
    ...
    ourtmp_dir = os.path.join(work_dir, "pki.tmp." + Cnv.tohex(Rng.bytes(4)))
    os.mkdir(ourtmp_dir)
...
def test_cipher_block():
    ...
    key = Rng.bytes(Cipher.keybytes(Cipher.Alg.TDEA))
    print "KY:", Cnv.tohex(key)
...
def test_cipher_file():
    ...
    iv = Rng.bytes(Cipher.blockbytes(Cipher.Alg.AES128))
    print "IV:", Cnv.tohex(iv)
...
def test_x509_generate():
    ...
    fname = 'newcert.cer'
    n = X509.save_file_from_string(
        fname, s, fileformat=X509.Opts.FORMAT_PEM)
...
def test_x509_validate():
    ...
    isok = X509.cert_path_is_valid(certlist, opts=X509.Opts.NO_TIMECHECK)
...
def test_x509_extract():
    ...
    n = X509.get_cert_count_from_p7(p7file, 0)
