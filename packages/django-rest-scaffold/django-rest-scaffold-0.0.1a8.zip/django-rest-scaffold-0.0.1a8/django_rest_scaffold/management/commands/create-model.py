...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(views_path) or 'w' in file_mode:
            ...
            views_lines.append('from resources.authentication.scope.permissions import ScopePermission\n')
            views_lines.append('from resources.authentication.user.authentication import AccessTokenAuthentication\n')
            views_lines.append('from rest_framework import generics, permissions\n')
        else:
            views_lines = []
        views_lines += [
            '\n\n',
            ...
            "    scopes = {{'get': '{0}-list', 'post': '{0}-create'}}\n".format(model_name.lower()),
            '\n\n',
            ...
            '    authentication_classes = (AccessTokenAuthentication,)\n',
            '    permission_classes = (permissions.IsAuthenticated, ScopePermission)\n',
            "    scopes = {{'get': '{0}-retrieve', 'put': '{0}-update', 'patch': '{0}-update', 'delete': '{0}-delete'}}\n".format(model_name.lower()),
        ]
