...
class Auth(object):
    ...
    def get_login_user(self):
        ...
        ukey = "%s:Auth:UserInfo:%s" % (self.app.config["SSO_APP_ID"], token)
    ...
    def load_user(self):
        ...
        if token:
            ukey = "%s:Auth:UserInfo:%s" % (self.app.config["SSO_APP_ID"], session.get('user_token'))
            user_info = self.cache.get(ukey)
            if user_info:
                g.user = user_info
            ...
            if secretid and signature:
                ukey = "%s:Auth:UserInfo:%s" % (self.app.config["SSO_APP_ID"], secretid)
                user_info = self.cache.get(ukey)
                if user_info:
                    g.user = user_info
                    if token_re.findall(request.url):
                ...
                return res_json(
                    code=401,
                    data="",
                    message="auth fail,x-secretid and x-signature or x-token in headers or need login.",
                    success=False
                )
