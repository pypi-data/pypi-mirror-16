    ...
    def _create_parameters(self):
        ...
        for i in range(self.p):
            if self.p > 1:
                ...
                self.parameters.parameter_list[i].start = 0.42

        for i in range(self.q,self.p+self.q):
            if self.q > 1:
                self.parameters.parameter_list[i].start = 0.0001
            elif self.q == 1:
                self.parameters.parameter_list[i].start = 0.07

        for i in range(self.p+self.q,self.param_no):
            self.parameters.parameter_list[i].start = 0.0001

        self.parameters.parameter_list[self.p+self.q].start = 2.0
        self.parameters.parameter_list[self.p+self.q+2].start = np.log(0.02)
        ...
        for t in range(0,Y.shape[0]):
            ...
            if t < self.max_lag:
                ...
                for q_term in range(0,self.q):
                    lmda[t] += parm[self.p+p_term]*scores[t-q_term-1]
                ...
                theta[t] = np.dot(self.X[t],parm[-len(self.X_names):]) + parm[-(len(self.X_names)*2)-1]*np.exp(lmda[t-1])
