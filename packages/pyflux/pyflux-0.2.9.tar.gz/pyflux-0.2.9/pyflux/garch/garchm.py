...
class GARCHM(tsm.TSM):
    """ Inherits time series methods from TSM class.

    **** GENERALIZED AUTOREGRESSIVE CONDITIONAL HETEROSCEDASTICITY in mean (GARCH-M) MODELS ****
    ...
    def __init__(self,data,p,q,target=None):
        ...
        super(GARCHM,self).__init__('GARCHM')
        ...
        self.model_name = "GARCH-M(" + str(self.p) + "," + str(self.q) + ")"
    ...
    def _create_parameters(self):
        ...
        for k in range(1,len(self.parameters.parameter_list)-1):
            if k > self.q:
                self.parameters.parameter_list[k].start = 0.90
            else:
    ...
    def _model(self,beta):
        ...
        eps = np.ones(Y.shape[0])
        sigma2 = np.ones(Y.shape[0])*parm[0]
            ...
            if t < self.max_lag:
                ...
                if self.q != 0:
                    for i in range(0,self.q):
                        sigma2[t] += parm[1+i]*eps[t-1-i]
                # GARCH terms
                if self.p != 0:
                    for i in range(0,self.p):
                        sigma2[t] += parm[1+self.q+i]*sigma2[t-1-i]

            eps[t] = Y[t] - parm[-2] - parm[-1]*sigma2[t]
        return sigma2, Y, eps
    ...
    def neg_loglik(self,beta):
        ...
        parm = np.array([self.parameters.parameter_list[k].prior.transform(beta[k]) for k in range(beta.shape[0])])
        return -np.sum(ss.norm.logpdf(Y,loc=(parm[-2]*np.ones(sigma2.shape[0]))+parm[-1]*sigma2,scale=np.power(sigma2,0.5)))
    ...
    def plot_fit(self,**kwargs):
        ...
        if self.parameters.estimated is False:
            ...
            plt.subplot(2, 1, 2)
            sigma2, Y, ___ = self._model(self.parameters.get_parameter_values())
            plt.plot(date_index,Y,label=self.data_name + ' Series')
            plt.plot(date_index,(t_params[-2]+(t_params[-1]*sigma2)),label='Series',c='black')
            plt.title(self.data_name + " GARCH-M Series")  
            plt.legend(loc=2)               
            plt.show()              
