...
def _parse_target_arguments(target, rule, argv):
    ...
    desc = rule._rule.get_description()
...
def main(argv=sys.argv[1:], **kwargs):
    ...
    if target is not None:
        ...
        if success:
            core.write_stdout(info, target, results["updated"], results["total"], results["time"])
        else:
            core.write_stderr(info, target, results["updated"], results["total"], results["time"])
