...
import logging
import colorama
from . import (NAME, VERSION, DESCRIPTION, INPUT_FILE)
...
class Core(object):
    ...
    `name`
        Package name string, defaults to package definition.
    `version`
        Package version string, defaults to package definition.
    `description`
        Package description string, defaults to package definition.
    `stdout`
    ...
    def __init__(self, name=NAME, version=VERSION, description=DESCRIPTION,
        ...
        # Standard output/error stream attributes, defaults to streams
        # wrapped by colorama.
        # TODO: Fix this.
        self._stdout = stdout if stdout else sys.stdout
        self._stderr = stderr if stderr else sys.stderr
    ...
    def _write_stream(self, stream, fmt, *args, **kwargs):
        ...
        if raw:
            fmt = "{}{}{}\n".format(style, fmt, colorama.Style.RESET_ALL)
        else:
            fmt = "{}{}: {}{}\n".format(
                style, self.get_name(), fmt, colorama.Style.RESET_ALL)
