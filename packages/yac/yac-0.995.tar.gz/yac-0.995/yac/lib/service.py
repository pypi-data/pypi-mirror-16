...
def get_service_parmeters(service_alias, service_params_via_cli, 
    ...
    # Before doing a bunch of work to define the service parameters for this
    # service, see if we cached the subnet info on a previous run of this fxn.
    param_cache_key = "%s:%s"%(service_name,PARAMS_CACHE_SUFFIX)
    service_parmeters = get_cache_value(param_cache_key)

    if service_parmeters:
        ...
        if use_cached_inputs and use_cached_inputs=='n':
            # reset params
            service_parmeters = {}

    if not service_parmeters:

        # gather params
        service_parmeters = regather_service_parmeters(service_alias, service_params_via_cli, 
                                  service_name, service_descriptor,
                                  servicefile_path, vpc_prefs)
...
def regather_service_parmeters(service_alias, service_params_via_cli, 
                          service_name, service_descriptor,
                          servicefile_path, vpc_prefs={}):
