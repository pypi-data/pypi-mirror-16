...
parser = argparse.ArgumentParser(description='Walks over a directory to apply the .svnignore file')
...
parser.add_argument(
	...
	action='store_true',
	default=False,
	help='Overwrite the existing ignore property. Disables the recursive option.',
	required=False
)
...
for directory, subDirectories, files in os.walk(args.directory):
	...
	if args.ignore_file in files:
		...
		if(process.communicate()[1]):
			logger.error(str(process.communicate()[1], 'utf-8'))