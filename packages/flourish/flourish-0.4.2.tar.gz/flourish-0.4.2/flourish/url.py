...
class URL(object):
    ...
    def get_all_filters_for_arguments(self, keys, objects=None):
        # FIXME this should not be a URL method, it doesn't depend on
        # any specific URL
        if objects is None:
            objects = self.parent
        ...
        for _source in objects:
            _is_date_filter = (
                _first_key in ['year', 'month', 'day'] and
            ...
            if _is_date_filter:
                ...
                # _values.add(getattr(_source['published'], _first_key))
                # if _first_key == 'year':
                #     _values.add('dateyear', _first_key)
                # elif _first_key == 'month':
                #     _values.add('datemonth', '%02s' % _first_key)
                # else:
                #     _values.add('dateday', '%02s' % _first_key)
