...
def generate_once(args):
    # FIXME report a nicer error if this doesn't exist
...
def generate_on_change(args):
    ...
    # FIXME static content dir should be watched for regen too
    observer.schedule(Handler(), args.source, recursive=True)
    observer.schedule(Handler(), args.templates, recursive=True)
    observer.schedule(Handler(), args.static, recursive=True)
    # FIXME explodes too easily, should do source not in bg thread?
    observer.start()
    try:
        while True:
            time.sleep(0.5)
...
def create_example(args):
    for _dir in ['source', 'templates', 'static']:
