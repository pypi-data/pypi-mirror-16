...
//		result = PyList_New(object->nParts);
			 ...
			 part_idx++)
		{
			if (part_idx < object->nParts - 1)
				length = (object->panPartStart[part_idx + 1] - object->panPartStart[part_idx]);
			else
...
//			if (PyList_SetItem(result, part_idx, part) < 0)
			if (PyTuple_SetItem(result, part_idx, part) < 0)
				goto fail;
...
//		if (PyList_SetItem(list, i, vertex) < 0)
		if (PyTuple_SetItem(list, i, vertex) < 0)
			goto fail;
    }
			  ...
			  PyObject * part_type_list)
{
    ...
    if (part_type_list 
		&& num_parts != PySequence_Length(part_type_list))
    {
		int seq_num = PySequence_Length(part_type_list);
		char *temp = (char *) malloc(100);
		sprintf(temp, "parts=%d and part_types=%d of id=%d have to have the same lengths", 
			num_parts, seq_num, id);
		PyErr_SetString(PyExc_TypeError, temp);
		free((char *) temp);
		return NULL;
    }
...
static PyObject *_wrap_SHPObject_count_get(PyObject *self, PyObject *args) {
    PyObject *resultobj;
    SHPObject *arg0 ;
    PyObject * argo0 =0 ;
    int result ;
    
    if(!PyArg_ParseTuple(args,"O:SHPObject_count_get",&argo0)) return NULL;
    if ((SWIG_ConvertPtr(argo0,(void **) &arg0,SWIGTYPE_p_SHPObject,1)) == -1) return NULL;
    result = (int ) (arg0->nVertices);
    resultobj = PyInt_FromLong((long)result);
    return resultobj;
}

static PyObject *_wrap_SHPObject_parttypes_get(PyObject *self, PyObject *args) {
    PyObject *resultobj;
    SHPObject *arg0 ;
    PyObject * argo0 =0 ;
	PyObject *result;
    ...
    if(!PyArg_ParseTuple(args,"O:SHPObject_parttypes_get",&argo0)) return NULL;
    if ((SWIG_ConvertPtr(argo0, (void **) &arg0, SWIGTYPE_p_SHPObject, 1)) == -1) return NULL;
	result = PyList_New(arg0->nParts);
	if (!result) return NULL;
	
	for (part_idx=0; part_idx < arg0->nParts; part_idx++)
	{
		int pt = arg0->panPartType[part_idx];
		if (PyList_SetItem(result, part_idx, PyInt_FromLong((long)(pt))) < 0)
			goto fail;
	}
    return result;
...
static PyMethodDef shapelibcMethods[] = {
	 ...
	 { "SHPObject_count_get", _wrap_SHPObject_count_get, METH_VARARGS },
	 { "SHPObject_parttypes_get", _wrap_SHPObject_parttypes_get, METH_VARARGS },
