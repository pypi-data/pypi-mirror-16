...
class SkyWiseResource(object):
    ...
    @classmethod
    def find_by_id_async(cls, resource_id):
        url = cls._resource_path(resource_id)
        if cls._use_session_for_async:
            greq = grequests.get(url, session=cls._s(), headers=cls.get_headers())
        else:
            greq = grequests.get(url, auth=HTTPBasicAuth(cls.get_user(), cls.get_password()), headers=cls.get_headers())
        return SkyWiseRequest(greq, cls)

    @classmethod
    def find_async(cls, id_=None, **kwargs):

        if id_:
            return cls.find_by_id_async(id_)

        # Request a list of resources
        url = cls._list_path(**kwargs)
        args = cls._path_args(**kwargs)
        if cls._use_session_for_async:
            r = grequests.get(url, session=cls._s(), headers=cls.get_headers())
        else:
            r = grequests.get(url, auth=HTTPBasicAuth(cls.get_user(), cls.get_password()), headers=cls.get_headers(), params=args)
    ...
    @classmethod
    def _get(cls, id_, **kwargs):
        url = cls._resource_path(id_, **kwargs)
        args = cls._path_args(**kwargs)
        res = cls._s().get(url, params=args, headers=cls.get_headers())
        if res.status_code != 200:
            try:
