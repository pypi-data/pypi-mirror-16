...
ASSETS_DIR = resource_string(__name__, 'assets')
...
red = GdkPixbuf.Pixbuf.new_from_file(resource_string(ASSETS_DIR, 'r.jpg'))
green = GdkPixbuf.Pixbuf.new_from_file(resource_string(ASSETS_DIR, 'g.jpg'))
...
class PythonPatternHandler(PatternMatchingEventHandler):
    ...
    def on_modified(self, event):
        ...
        if process.returncode == 0:
            notification.update('OK TO TALK', '', resource_string(ASSETS_DIR, 'g.jpg'))
            notification.set_image_from_pixbuf(green)
            print('Tests passing!')
        else:
            notification.update('NOT OK TO TALK', '', resource_string(ASSETS_DIR, 'r.jpg'))
