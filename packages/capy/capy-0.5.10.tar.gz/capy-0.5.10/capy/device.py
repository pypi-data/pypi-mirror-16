...
class BaseDevice(object):
    ...
    def show_and_run_commands(self, base_cmd, test):
        temp_dir = self.report_dir('.capy_temp/', create=True)
        dir = self.report_dir(self.platform.output_dir)

        cmd = base_cmd + test.create_command(temp_dir)
        # show commands
        print '--------------------------------------------------------------------------'
        print '| Commands: '
        print '|'
        print '|', " ".join(cmd)
        print '|'
        print '| NOTE: output files will be moved to:', dir
        print '|'
        print '--------------------------------------------------------------------------'

        # run command
        self.ENV["SCREENSHOT_PATH"] = temp_dir + '/'  # has to end with '/'
        self.call(cmd)
    ...
    def report_dir(self, parent=None, create=False):
