...
def merge_dt(mutable_dt, dt):
    ndt = datetime(year=mutable_dt[0] if mutable_dt[0] > -1 else dt.year,
                   month=mutable_dt[1] if mutable_dt[1] > -1 else dt.month,
                   day=mutable_dt[2] if mutable_dt[2] > -1 else dt.day,
                   hour=mutable_dt[3] if mutable_dt[3] > -1 else dt.hour,
                   minute=mutable_dt[4] if mutable_dt[4] > -1 else dt.minute,
                   second=mutable_dt[5] if mutable_dt[5] > -1 else dt.second,
                   microsecond=mutable_dt[6] if mutable_dt[6] > -1 else dt.microsecond)
    return ndt
...
def handle_meta_range(cleaned_bundle, future, locale, text):
    ...
    if not metadates:
        ...
        # generic
        dt, edt = resolve_dt(metadates, now)
