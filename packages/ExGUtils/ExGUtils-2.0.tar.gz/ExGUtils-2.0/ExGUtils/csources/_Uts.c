...
static char Function_maxLKHD_docstring[] = "Finds the parameters that maximize the likelyhood for the values in a list.\n\
                                            Usage: [mu, sig, tau] = maxLKHD(lista, mu=0., sig=0., tau=0., lamb=1., eps=1.e-16)\n\
                                            ...
                                            and eps is the precision (the interactions stop when the absolute value of the gradient is smaller than eps).";
...
static char Function_minSQR_docstring[] = "Finds the parameters that minimize the sum of the squares. \n\
                                           Usage: [mu, sig, tau] = minSQR(listax, listay, mu=0., sig=0., tau=0., lamb=1., eps=1.e-16)\n\
                                           The first three optional arguments are the initial point for searching (actually if the three \n\
                                           are zero, it evaluates mu, sig and tau with respect to the distribution average, standard deviation \n\
                                           and assymmetry), lamb is the the starting update in the parameters with respect to the gradient module\n\
                                           and eps is the precision (the interactions stop when the absolute value of the gradient is smaller than eps).\n\
                                           Note that listax and listay should represent a normalized histogram of the data!";
static char Function_exgauss_lt_docstring[] = "Left-tail of the exgaussian distribution\n\
                                               Usage: alp = exgauss_lt(z, mu, sig, tau, eps=1.e-12)\n\
                                               The left tail is the integral of the function between minus infinity and z.\n\
                                               The optional argument eps is the precision of the result.";
static char Function_exgauss_lamb_lt_docstring[] = "Left-tail of the exgaussian distribution parametrized only in terms of lambda\n\
                                                    Usage: alp = exgauss_lamb_lt(z, lamb, eps=1.e-12)\n\
                                                    The left tail is the integral of the function between minus infinity and z.\n\
                                                    The optional argument eps is the precision of the result.";
static char Function_zalp_exgauss_docstring[] = "Point that leaves a left tail of alp in the exgaussian\n\
                                                 Usage: z = zalp_exgauss(alp, mu, sig, tau, eps=1.e-12)\n\
                                                 The left tail is the integral of the function between minus infinity and z.\n\
                                                 The optional argument eps is the precision of the result.";
static char Function_zalp_exgauss_lamb_docstring[] = "Point that leaves a left tail of alp in the exgaussian parametrized by lambda\n\
                                                      Usage: z = zalp_exgauss_lamb(alp, lamb, eps=1.e-12)\n\
                                                      The left tail is the integral of the function between minus infinity and z.\n\
                                                      The optional argument eps is the precision of the result.";
...
static PyObject *Function_exgauss_lt(PyObject *self, PyObject *args, PyObject *kwds);
static PyObject *Function_exgauss_lamb_lt(PyObject *self, PyObject *args, PyObject *kwds);
static PyObject *Function_zalp_exgauss(PyObject *self, PyObject *args, PyObject *kwds);
static PyObject *Function_zalp_exgauss_lamb(PyObject *self, PyObject *args, PyObject *kwds);
...
static PyMethodDef module_methods[] = {
    ...
    {"drand_exp", (PyCFunction)Function_drand_exp, METH_VARARGS, Function_drand_exp_docstring},
    {"drand_gauss", (PyCFunction)Function_drand_gauss, METH_VARARGS | METH_KEYWORDS, Function_drand_gauss_docstring},
    {"drand_exg", (PyCFunction)Function_drand_exg, METH_VARARGS, Function_drand_exg_docstring},
    {"gaussian", (PyCFunction)Function_gaussian, METH_VARARGS, Function_gaussian_docstring},
    {"exgauss", (PyCFunction)Function_exgauss, METH_VARARGS, Function_exgauss_docstring},
    {"exgauss_lamb", (PyCFunction)Function_exgauss_lamb, METH_VARARGS, Function_exgauss_lamb_docstring},
    ...
    {"exgauss_lt", (PyCFunction)Function_exgauss_lt, METH_VARARGS | METH_KEYWORDS, Function_exgauss_lt_docstring},
    {"exgauss_lamb_lt", (PyCFunction)Function_exgauss_lamb_lt, METH_VARARGS | METH_KEYWORDS, Function_exgauss_lamb_lt_docstring},
    {"zalp_exgauss", (PyCFunction)Function_zalp_exgauss, METH_VARARGS | METH_KEYWORDS, Function_zalp_exgauss_docstring},
    {"zalp_exgauss_lamb", (PyCFunction)Function_zalp_exgauss_lamb, METH_VARARGS | METH_KEYWORDS, Function_zalp_exgauss_lamb_docstring},
...
static PyObject *Function_pars_to_stats(PyObject *self, PyObject *args) {
    ...
    return Py_BuildValue("ddd", M, S, lamb);
}
...
static PyObject *Function_stats_to_pars(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "ddd", &M, &S, &lamb))
        return NULL;
    if ((lamb<=0.)|(lamb>=1.)) {
        printf("Warning: lambda must be in the interval (0; 1)!\n");
        //return Py_BuildValue("");
...
static PyObject *Function_exgauss_lt(PyObject *self, PyObject *args, PyObject *kwds) {   
    ...
    // Parse the input
    static char *kwlist[] = {"z", "mu", "sig", "tau", "eps", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "dddd|d", kwlist, &z, &mu, &sig, &tau, &eps))
        return 0;
    
    lt = exgauss_lt(z, mu, sig, tau, eps, 50);
    return Py_BuildValue("d", lt);
}
...
static PyObject *Function_exgauss_lamb_lt(PyObject *self, PyObject *args, PyObject *kwds) {   
    ...
    // Parse the input
    static char *kwlist[] = {"z", "lamb", "eps", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "dd|d", kwlist, &z, &lamb, &eps))
        return 0;
    
    lt = exgauss_lamb_lt(z, lamb, eps, 10);
    return Py_BuildValue("d", lt);
}
...
static PyObject *Function_zalp_exgauss(PyObject *self, PyObject *args, PyObject *kwds) {   
    double alp, mu, sig, tau;
    double eps=1.e-12;
    double zzz;
    
    // Parse the input
    static char *kwlist[] = {"alp", "mu", "sig", "tau", "eps", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "dddd|d", kwlist, &alp, &mu, &sig, &tau, &eps))
        return 0;
    
    if ( (alp>1.)|(alp<0.) ) {
        printf("Warning: alp must be a number between 0. and 1.!\n");
        return Py_BuildValue("");
    } else {
        zzz = zalp_exgauss(alp, mu, sig, tau, eps);
        return Py_BuildValue("d", zzz);
    }
}
...
static PyObject *Function_zalp_exgauss_lamb(PyObject *self, PyObject *args, PyObject *kwds) {   
    ...
    // Parse the input
    static char *kwlist[] = {"alp", "lamb", "eps", NULL};
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "dd|d", kwlist, &alp, &lamb, &eps))
        return 0;
    
    zzz = zalp_exgauss_lamb(alp, lamb, -8., 1., eps);
    return Py_BuildValue("d", zzz);
}
