...
class ConfigParser(ConfigParser.ConfigParser):
    UNNAMED_SECTION = u'#--ConfigParser--INTERNAL--UNNAMED-SECTION--#'
    # It will transfer to empty line
    __EMPTY_OPTION = u'--ConfigParser--INTERNAL--EMPTY-SECTION--'
    # It will transfer to comment line
    __COMMENT_OPTION = u'--ConfigParser--INTERNAL--COMMENT-SECTION--'
    ...
    def set(self, section, option, value):
        ...
        if issubclass(ConfigParser, object):
            super(ConfigParser, self).set(section, option, unicode(value))
        else:
            ConfigParser.ConfigParser.set(self, section, option, unicode(value))

    def readfp(self, fp, *args, **kwargs):
        # if hasattr(fp, 'name'):
        # We could only use the readline() by definitions
        abuffer = u'[' + self.UNNAMED_SECTION + u']' + os.linesep
        ...
        while True:
            ...
            if len(temp_line) <= 0:
                line = self.__EMPTY_OPTION + unicode(i) + u'=#' + os.linesep
            elif temp_line.startswith(u'#') or temp_line.startswith(u';'):
                line = self.__COMMENT_OPTION + unicode(i) + u'=#' + line
            abuffer += line
