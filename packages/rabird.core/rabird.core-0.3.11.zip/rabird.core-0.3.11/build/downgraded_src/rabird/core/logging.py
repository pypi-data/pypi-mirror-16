u'''
@date 2013-5-9

@author Hong-She Liang <starofrainnight@gmail.com>
'''

from __future__ import absolute_import
import sys
import os

# Import the global logging unit, not our logging .
global_logging = __import__(u'logging')


def load_default_config():
    arguments = {
        u'level': None,
        u'filename': None,
        u'filemode': None,
        u'format': None,
        u'datefmt': None,
        u'style': None,
    }

    for k in list(arguments.keys()):
        try:
            envionment_text = u'PYTHON_LOGGING_{}'.format(k.upper())
            arguments[k] = os.environ[envionment_text]
        except ValueError:
            pass
        except KeyError:
            pass

    # Remove all arguments that is None value.
    keys = list(arguments.keys())
    for k in keys:
        if arguments[k] is None:
            del arguments[k]

    # Set default level to logging.INFO .
    if u'level' not in list(arguments.keys()):
        arguments[u'level'] = global_logging.INFO

    global_logging.basicConfig(**arguments)

    # Added console handler only there have filename argument.
    if u'filename' in list(arguments.keys()):
        global_logging.getLogger().addHandler(global_logging.StreamHandler(sys.stdout))
