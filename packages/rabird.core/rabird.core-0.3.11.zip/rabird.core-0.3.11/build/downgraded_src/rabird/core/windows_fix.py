...
class StdoutThread(threading.Thread):
    ...
    def __init__(self, std_handle_type, old_stdout):
        ...
        self.stdout = io.open(self.file_descriptor, mode=u'rb')
        self.stdin = None

    def run(self):
        self.stdin = StdioFile(self.stdout_pipe[1], u"wb")
                ...
                if(type(s) == unicode):
                    if self.screen_buffer is not None:
                        ...
                        try:
                            ...
                            self.old_stdout.write(unicode(s))
                    else:
                        self.old_stdout.write(unicode(s))
                else:
...
class StdioFile(io.FileIO):
    ...
    def write(self, value):
        if(type(value) != unicode):
            value = unicode(value)  # changed value to str

        # the line separator must be at the end of line !
        value = unicode(pickle.dumps(value)) + os.linesep
...
def monkey_patch():
    ...
    if u'StdioFile' not in unicode(type(sys.stdout)):
        stdout_thread = StdoutThread(win32api.STD_OUTPUT_HANDLE, old_stdout)
        stdout_thread.start()
        is_patched = True

    if u'StdioFile' not in unicode(type(sys.stderr)):
