...
anticipated **state of the lightbulb** at 7:15 if there is 2 persons in
the room ?".
...
-  ``timezone`` properties can take string values representing the
   ...
   ``+01:30``)), between ``-12:00`` and ``+14:00``.
    ...
    :information\_source: By default, the values of ``time_of_day`` and
    ``day_of_week`` properties are > generated from the
    ```timestamp`` <#timestamp>`__ of an agent's state and the agent's
    current > ``timezone``.

    If you wish to provide their value manually, add
    ``is_generated: false`` to the time types in your model. In this
    case, since you provide the values, you must update the context
    whenever one of these time values changes in a way that is
...
.. |Build Status| image:: https://travis-ci.org/craft-ai/craft-ai-client-python.svg?branch=master
