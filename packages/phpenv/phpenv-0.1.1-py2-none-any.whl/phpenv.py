...
def download_php_ini(env_dir, opt):
    url = 'https://raw.githubusercontent.com/php/php-src/PHP-{0}/php.ini-production'.format(opt.php)
    content = urllib2.urlopen(url).read()
    php_ini_path = '{0}/etc/php.ini'.format(env_dir)
    if opt.etc_dir:
        php_ini_path = '{0}/php.ini'.format(opt.etc_dir) 

    with open(php_ini_path , 'wb') as file:
        file.write(content)
...
def parse_args():
    ...
    parser.add_option('--with-mysql', action='store_true',
            dest='with_mysql', default=False,
            help='Compile php with mysql support')

    parser.add_option('-v', '--verbose', action='store_true',
            dest='verbose', default=False,
            help='Verbosity output')

    parser.add_option('--etc-dir', action='store',
            dest='etc_dir', default=None,
            help='Override etc directory with presented')
...
def build_php_from_src(env_dir, src_dir, opt):
    ...
    if opt.etc_dir:
        conf_cmd.append('--with-config-file-path=%s' % pipes.quote(opt.etc_dir))
        conf_cmd.append('--with-config-file-scan-dir=%s' % pipes.quote(os.path.join(opt.etc_dir, 'php.conf.d')))
        conf_cmd.append('--sysconfdir=%s' % pipes.quote(opt.etc_dir))
    else:
        conf_cmd.append('--with-config-file-path=%s' % pipes.quote(os.path.join(env_dir, 'etc')))
        conf_cmd.append('--with-config-file-scan-dir=%s' % pipes.quote(os.path.join(env_dir, 'etc', 'php.conf.d')))
    
    conf_cmd.append('--with-layout=GNU')
...
def install_phpext(env_dir, opt):
    
    mode_0755 = (stat.S_IRWXU | stat.S_IXGRP |
                 stat.S_IRGRP | stat.S_IROTH | stat.S_IXOTH)

    with open('{0}/bin/phpext'.format(env_dir), 'wb') as destfile:
        destfile.write(PHPEXT)
    os.chmod('{0}/bin/phpext'.format(env_dir), mode_0755 )
...
def create_environment(env_dir, opt):
    ...
    src_dir = os.path.abspath(os.path.join(env_dir, 'php-src'))
    mkdir(src_dir)
    
    if opt.etc_dir:
        opt.etc_dir = os.path.realpath(opt.etc_dir)
        if not os.path.exists(opt.etc_dir):
            mkdir(opt.etc_dir)

    install_php(env_dir, src_dir, opt)
    #mkdir(os.path.join(env_dir, 'bin'))
    install_activate(env_dir, opt)
    install_phpext(env_dir, opt)
    download_php_ini(env_dir, opt)
...
if [ -z $PHP_VIRTUAL_ENV ];
then
	echo "Php virtual env not presented"
	echo "this script must be run in phpenv only"
	exit 2
fi

if [ ! -d "$PHP_VIRTUAL_ENV/php-src/" ];
then
	echo "Php sources dir not found"
	
	if [ -f "$PHP_VIRTUAL_ENV/php-src.tar.gz" ];
	then
		echo "Found sources tar, using it"
		tar -xf $PHP_VIRTUAL_ENV/php-src.tar.gz -C $PHP_VIRTUAL_ENV/php-src
	else
		echo "Php sources tar not found too, exit"
		exit 2
	fi
fi

PHP_SRC_DIR=$(bash -c "cd $PHP_VIRTUAL_ENV/php-src/php-* && pwd")

if [ $# -lt 1 ]; then
	echo "Usage: phpext <extension> [CONFIGURE OPTIONS]"
	echo "    or phpext -l "
	exit 1
fi
...
if [ "$1" == "-l" ];
then
	for file in $(ls -l "$PHP_SRC_DIR/ext/" | grep -E "^d" | awk '{print $9}');
	do
		echo $file
	done
	exit
...
if [ ! -d $PHP_VIRTUAL_ENV/php-src/php-*/ext/$EXTENSION ];
then
	echo "Extension $EXTENSION not found"
	exit 2
fi

cd $PHP_VIRTUAL_ENV/php-src/php-*/ext/$EXTENSION
phpize
CPPFLAGS="-I$PHP_SRC_DIR" CFLAGS="-I$PHP_SRC_DIR" ./configure $CONFIGURE_OPTIONS
make
make install
"""
