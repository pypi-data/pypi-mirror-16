...
class AWS4Auth(AuthBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        session_token_expiration
                    -- datetime.datetime instace for session token expiration
        next_keys
                    -- Callback for regenerating current keys after expiration.
        ...
        if isinstance(args[1], AWS4SigningKey) and l == 2:
            self._initialize_from_signing_key(
                access_id=args[0],
                signing_key=args[1],
                **kwargs
            )
    ...
    def _initialize(self, access_id, secret_key, region, service, date, **kwargs):
    ...
    def _initialize_from_signing_key(self, access_id, signing_key, **kwargs):
        self.access_id = access_id
    ...
    def _initialize_settings(self, **kwargs):
        ...
        if 'session_token_expiration' in kwargs:
            expiration_dt = kwargs.get('session_token_expiration')
            self.session_token_expiration = timestamp(expiration_dt)

            self.next_keys = kwargs.get('next_keys')
            if not self.next_keys:
                raise ValueError('next_keys callback needs to be provided with session_token_expiration in AWS4Auth.__init__()')
    ...
    def __call__(self, req):
        ...
        if self._session_token_expired:
            logging.info("AWS4Auth.__call__() - session token expired, renewing keys")
            self._renew_keys()
    ...
    @property
    def _session_token_expired(self):
        return self.session_token_expiration and self.session_token_expiration <= timestamp(datetime.datetime.utcnow())

    def _renew_keys(self):
        keys = self.next_keys()
        settings = dict(
            raise_invalid_date=keys.get('raise_invalid_date') or False,
            session_token=keys.get('session_token'),
            session_token_expiration=keys.get('session_token_expiration'),
            next_keys=keys.get('next_keys'),
        )

        self._initialize(
            access_id=keys.get('access_id'),
            secret_key=keys.get('secret_key'),
            region=keys.get('region'),
