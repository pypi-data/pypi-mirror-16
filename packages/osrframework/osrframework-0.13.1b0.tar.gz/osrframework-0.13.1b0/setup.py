...
while installationFound:
    try:
        import osrframework
        major, minor, micro = osrframework.__version__.split('.')
        if int(major) == 0 and int(minor) <= 12:
            # We will manually remove previous installations!
            importDetails = imp.find_module("osrframework")
            try:
                print "[!] Deprecated installation found at: " + importDetails[1]
                shutil.rmtree(importDetails[1])
                print "[*] Successfully removed. Installation will resume now..."
            except Exception as e:
                print str(e)
                print "[E] OSRFramework is installed, but cannot be removed. Try to remove it manually in your python installation under 'local/lib/python2.7/dist-packages/'."
                print sys.exit()
        else:
            print "[*] OSRFramework is installed, but with a compatible version."
            break
    except Exception as e:
        print str(e)
...
############################
print "[*] Recursively changing permissions..."
try:
