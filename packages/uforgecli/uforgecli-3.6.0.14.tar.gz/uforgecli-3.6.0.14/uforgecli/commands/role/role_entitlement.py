...
class Role_Entitlement_Cmd(Cmd, CoreGlobal):
        ...
        def arg_remove(self):
                ...
                mandatory.add_argument('--name', dest='name', required=True, help="the name of role")
                mandatory.add_argument('--entitlements', dest='entitlements', nargs='+', required=True, help="the login name to remove as a new subscription profile administrator. You can use Unix matching system (*,?,[seq],[!seq]) and multiple match separating by space.")
                optional.add_argument('--org', dest='org', required=False, help="the organization name. If no organization is provided, then the default organization is used.")
                return doParser
