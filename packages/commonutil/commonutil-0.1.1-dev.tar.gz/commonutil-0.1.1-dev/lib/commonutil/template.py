...
class BaseLineBlockedMacroMarker(object):
	...
	def __init__(self, *args, **kwds):
		super(BaseLineBlockedMacroMarker, self).__init__(*args, **kwds)
	# ### def __init__
	...
	# ### def get_block_line_markers
# ### class BaseLineBlockedMacroMarker
...
class BaseLineDigestMarker(object):
	...
	def __init__(self, *args, **kwds):
		super(BaseLineDigestMarker, self).__init__(*args, **kwds)
	# ### def __init__
	...
	# ### def get_digest_line_marker
# ### class BaseLineDigestMarker


def _iter_context_line_innerloop(content, max_depth=8):
	for l in content:
		if (max_depth > 0) and isinstance(l, (list, tuple, set,)):
			for ll in _iter_context_line_innerloop(l, max_depth - 1):
				yield ll
			yield None
		else:
			yield l
# ### def _iter_context_line_innerloop
...
def _iter_digest_marked_context_line(content, digest_code, digest_marker):
	for l in _iter_context_line(content):
		if l is not None:
			...
			yield None
# ### def _iter_digest_marked_context_line
...
# ### def _render_flat_line_blocked_context
...
def render_line_blocked_context(existed_line_iterator, content_map, marker, gap_line_for_block_marker=False, keep_empty_content_marker=True):
	...
	Return:
		儲存有已放入巨集資料的字串串列
	"""
	return _render_flat_line_blocked_context(_iter_context_line(existed_line_iterator),
											content_map, marker, gap_line_for_block_marker, keep_empty_content_marker)
# ### def render_line_blocked_context


def fetch_content_from_line_blocked_context(existed_line_iterator, marker, keep_empty_line=False):
	""" 從既有已填充樣板資料的內容中取出巨集值
	...
	for l in existed_line_iterator:
		...
		if in_content_block:
			...
			if m is None:
				if is_trap_line:
					continue
				if (0 != len(l)) and (l[0:indent_len] == indent_text):
					l = l[indent_len:]
				if 0 == len(l):
					l = None
					if 0 == len(block_content):
						continue
			...
			if (indent_text, block_name,) != m:
				_log.warning("unequal block marker parameter: %r, %r != %r", indent_text, block_name, m)
			in_content_block = False
			while (len(block_content) > 0) and (not block_content[-1]):
				block_content.pop()
			...
			block_content = None
	return content_map
# ### def fetch_content_from_line_blocked_context
...
def put_content_to_line_blocked_file(filepath, content_map, marker, gap_line_for_block_marker=False, keep_empty_content_marker=True):
	...
	with open(filepath, "w") as fp:
		for l in result_context:
			fp.write(l + "\n")
# ### def put_content_to_line_blocked_file

def fetch_content_from_line_blocked_file(filepath, marker, keep_empty_line=False):
	""" 將巨集值由指定檔案中取出
	...
	"""
	with open(filepath, "r") as fp:
		content_map = fetch_content_from_line_blocked_context(fp, marker, keep_empty_line)
	return content_map
# ### def fetch_content_from_line_blocked_file


def replace_context_in_line_blocked_file(filepath, context_line_iterator, marker,
										gap_line_for_block_marker=False, keep_empty_content_marker=True, keep_empty_line=False):
	...
	if is_path_exists(filepath):
		...
		existed_content_map = {}
	result_context = render_line_blocked_context(
							context_line_iterator, existed_content_map, marker, gap_line_for_block_marker, keep_empty_content_marker)
	with open(filepath, "w") as fp:
		for l in result_context:
			fp.write(l + "\n")
# ### def replace_context_in_line_blocked_file
	...
	with open(filepath, "r") as fp:
		for l in fp:
			...
			if aux is not None:
				...
				return digest_code
	return None
# ### def fetch_digest_from_lined_file


def digest_checked_replace_context_in_line_blocked_file(filepath, context_line_iterator, digest_marker, macro_marker, gap_line_for_block_marker=False, keep_empty_content_marker=True):
	...
	if context_digest_code == existed_digest_code:
		return
	replace_context_in_line_blocked_file(filepath,
										_iter_digest_marked_context_line(context_line_iterator, context_digest_code, digest_marker),
										macro_marker, gap_line_for_block_marker, keep_empty_content_marker)
# ### def digest_checked_replace_context_in_line_blocked_file
