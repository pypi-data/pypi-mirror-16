...
def generate(env):
    ...
    if not hasattr(compiler, 'compiler'):   # assume Windows
        return generate_msvc(ext, compiler)
...
def generate_msvc(env, compiler):
    ...
    """
    env.Replace(CC=compiler.cc)
    env.Replace(CFLAGS=compiler.compile_options)
    env.Replace(CXX=compiler.cc)
    env.Replace(LINK=compiler.linker)   # wrong
    ...
    env.Replace(RC=compiler.rc) # also wrong    
