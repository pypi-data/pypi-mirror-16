...
这个模块包含了HTTP Adapters。尽管COS Python SDK内部使用requests库进行HTTP通信，但是对使用者是透明的。
...
from .exceptions import RequestError,ClientError
from .utils import file_object_remaining_bytes, SizedFileAdapter


_USER_AGENT = 'cos2-sdk-python/{0}({1}/{2}/{3};{4})'.format(
    ...
    def read(self, amt=None):
        if amt is None:
            ...
            for chunk in self.response.iter_content(_CHUNK_SIZE):
                content += chunk
            if(len(content)!=int(self.headers.get('content-length'))):
