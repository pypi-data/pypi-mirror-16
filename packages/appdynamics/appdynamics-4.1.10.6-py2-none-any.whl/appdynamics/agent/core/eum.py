...
from appdynamics.lib import iteritems, Morsel
...
def eum_metadata_injection_enabled(user_agent):
...
def inject_eum_metadata(bt, response_headers):
    ...
    """
    if not bt.eum_guid and eum_metadata_injection_enabled(bt.request.user_agent):
        bt.eum_guid = make_eum_guid()
        metadata = _generate_eum_metadata(bt)
