...
def evaluate_splitOp(node, ctx):
    ...
    if splitOp.firstNSegments:
        return split[:splitOp.firstNSegments]
    elif splitOp.lastNSegments:
        return split[-splitOp.lastNSegments:]
    elif splitOp.specificSegments:
        result = []
        for segment in splitOp.specificSegments:
            # Indexing is 1-based...
