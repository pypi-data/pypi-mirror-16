...
class BackendRegistry(object):
    ...
    exit_call_to_backend_type = {
        exitcalls.EXIT_HTTP: 'http',
        exitcalls.EXIT_DB: 'db',
        exitcalls.EXIT_CACHE: 'cache',
    }
    ...
    def backend_hash(self, exit_call_type, properties):
        ...
        """
        return (exit_call_type, tuple(sorted(items(properties))))
    ...
    def get_backend(self, exit_call_type, backend_properties, format_string):
        ...
        """
        backend_hash = self.backend_hash(exit_call_type, backend_properties)
        try:
            backend = self.backends[backend_hash]
        except KeyError:
            backend_type = self.exit_call_to_backend_type[exit_call_type]
            for entry in self.get_backend_config(backend_type):
                ...
                if not entry.HasField('matchRule') or \
                   self.match_backend(backend_properties, self.get_match_rule(backend_type, entry)):
                    ...
                    try:
                        backend = self.registered_backends[self.backend_hash(exit_call_type, identifying_properties)]
                    except KeyError:
                        ...
                        backend = exitcalls.Backend(exit_call_type, identifying_properties, display_name)
    ...
    def get_api_backend(self, exit_call_type, identifying_properties, display_name):
        ...
        """
        backend_hash = self.backend_hash(exit_call_type, identifying_properties)
        try:
    ...
    def get_db_backend(self, vendor, host, port, dbname):
        ...
        return self.get_backend(exitcalls.EXIT_DB, backend_properties, self.db_backend_naming_format_string)
    ...
    def update_config(self, backend_info, backend_detection_config):
        ...
        for rb in backend_info.registeredBackends:
            ...
            backend = exitcalls.Backend(exit_point_type, identifying_properties, exit_call_info.displayName)
            ...
            self.registered_backends[self.backend_hash(exit_point_type, identifying_properties)] = backend
