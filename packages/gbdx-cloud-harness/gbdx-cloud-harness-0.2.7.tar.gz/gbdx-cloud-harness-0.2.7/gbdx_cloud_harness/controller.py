...
class TaskController(object):
    ...
    def _run_app(self):
        ...
        if not isinstance(filename, str) and issubclass(filename, TaskTemplate):
            ...
            config_file = self._write_config_file(inspect.getfile(template_class))
    ...
    def _filter_archive(tarinfo):
        ...
        with open(ignore_filename, 'rb') as f:
            for ignore in f.readlines():
                ...
                if tarinfo.name.startswith(ignore) or tarinfo.name.endswith(ignore) or tarinfo.name.endswith('.pyc'):
                    return None
