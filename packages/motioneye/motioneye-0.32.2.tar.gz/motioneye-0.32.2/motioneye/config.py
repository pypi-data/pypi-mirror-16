...
def motion_camera_ui_to_dict(ui, old_config=None):
    ...
    data['ffmpeg_variable_bitrate'] = 2 + min(29, int(math.ceil(29.0 * (100 - int(ui['movie_quality'])) / 100.0)))
...
def motion_camera_dict_to_ui(data):
    ...
    if not bitrate or bitrate == 1:
        bitrate = 10 # about 75%

    ui['movie_quality'] = 100 - (bitrate - 2) * 100 / 29
...
#         binary, version = motionctl.find_motion()  # @UnusedVariable
#         
#         # all git versions are assumed to support mp4
