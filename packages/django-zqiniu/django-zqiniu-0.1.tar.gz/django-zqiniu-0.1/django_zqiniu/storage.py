...
class QiniuStorage(Storage):
    ...
    def __init__(self, bucket_config=None):
        ...
        assert 'BUCKET_URL' in settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config]
        assert 'BUCKET_NAME' in settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config]

        self.access_key = settings.QINIU_SETTINGS['ACCESS_KEY']
        self.secret_key = settings.QINIU_SETTINGS['SECRET_KEY']
        self.bucket_name = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config]['BUCKET_NAME']
        self.bucket_url = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config]['BUCKET_URL']
        self.bind_url = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config].get('BIND_URL', '')
        self.prefix = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config].get('PREFIX', '')
        self.is_private_bucket = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config].get('IS_PRIVATE', False)
        self.expires_time = settings.QINIU_SETTINGS['BUCKET_CONFIGS'][bucket_config].get('EXPIRES', 3600)
    ...
    def exists(self, name):
        ret, info = self.__bucket_manager.stat(self.bucket_name, self.prefix + name)
        return 'hash' in ret
    ...
    def listdir(self, prefix):
        if not prefix:
            prefix = self.prefix
        ret, eof, info = self.__bucket_manager.list(self.bucket_name, prefix=prefix)
        return ret
