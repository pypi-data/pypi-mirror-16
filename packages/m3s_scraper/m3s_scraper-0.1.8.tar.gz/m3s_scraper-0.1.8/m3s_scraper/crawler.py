    ...
    while blueX == "green":
        ...
        if len(words) > 600:
            try:
                for x in range(600):
                    wordly = words[x]
                    encodedlist.append(wordly.text + ' ')
        ...
        articleBufferString = (articleSource.text).encode('utf-8') + '|******' + articleBufferString

        articleBufferString = (articleDate.text).encode('utf-8') + '|' + articleBufferString
